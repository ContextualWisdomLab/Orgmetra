...
-- its own append-only tenant-scoped record. Attempt exhaustion is enforced at
-- every state-machine boundary so a dispatcher cannot manufacture attempt N+1.
    ...
    IF OLD.delivery_state_code = 'pending' THEN
        ...
        IF NEW.delivery_attempt_count <> OLD.delivery_attempt_count + 1
           ...
           OR NEW.last_failure_code IS DISTINCT FROM OLD.last_failure_code THEN
            RAISE EXCEPTION 'leasing an outbox delivery may only set lease metadata and increment attempt count once within budget'
                USING ERRCODE = '55000';
        END IF;
        RETURN NEW;
    END IF;
    ...
    IF OLD.delivery_state_code = 'leased' AND NEW.delivery_state_code = 'leased' THEN
        ...
        IF NEW.delivery_attempt_count <> OLD.delivery_attempt_count + 1
           ...
           OR NEW.last_failure_code IS DISTINCT FROM 'lease_expired' THEN
            RAISE EXCEPTION 'expired lease takeover requires a new future lease, one in-budget attempt increment, and lease-expired evidence'
                USING ERRCODE = '55000';
        END IF;
        RETURN NEW;
    END IF;
    ...
    IF OLD.delivery_state_code = 'leased' AND NEW.delivery_state_code = 'pending' THEN
        ...
        IF NEW.delivery_attempt_count <> OLD.delivery_attempt_count
           ...
           OR NEW.available_at < OLD.available_at THEN
            RAISE EXCEPTION 'retry transition requires cleared lease, stable attempt count, failure code, nondecreasing availability, and remaining attempt budget'
                USING ERRCODE = '55000';
        END IF;
        RETURN NEW;
    END IF;
...
-- Migration 0004 predates the durable retry budget. Replace its claim function
-- now that maximum_attempt_count exists so neither pending work nor an expired
-- lease can be claimed after the stored budget is exhausted.
...
-- Migration 0005 predates maximum_attempt_count. Recheck the budget under the
-- same row lock used for owner/lease validation so a final attempt cannot be
-- released back to pending and claimed as attempt N+1.
...
BEGIN
    ...
    IF current_lease_expires_at IS NULL THEN
        RAISE EXCEPTION 'outbox leased delivery has no lease expiry'
            USING ERRCODE = '55000';
    END IF;
    ...
    -- Once the final stored attempt is exhausted, normal reclaim is forbidden.
    -- The exact recorded owner may therefore terminalize an expired final lease
    -- as well as a live one; owner matching remains the capability boundary.
