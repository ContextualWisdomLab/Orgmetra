...
employment_leave_review_contract="${repository_root}/tests/test_employment_leave_review_artifact.sh"
...
if [[ ! -f "${employment_leave_review_contract}" ]]; then
  printf 'Foundation CI Employment Leave Review artifact contract is missing.\n' >&2
  exit 1
fi

bash "${employment_leave_review_contract}"
