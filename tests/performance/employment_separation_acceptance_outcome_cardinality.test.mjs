...
function validate(performance) {
  ...
  return validateEmploymentSeparationAcceptance(artifact, runtimeEvidence(artifact), FIXTURE_BYTES);
}

test("rejects perfect check rate when governed outcome checks cover only a subset of iterations", () => {
  const performance = result();
  performance.k6.metrics.checks.values.passes = 1;
  assert.throws(() => validate(performance), /checks.*expected_iterations|governed outcome checks/i);
});

test("rejects zero unexpected-response rate when governed outcome observations cover only a subset of iterations", () => {
  const performance = result();
  performance.k6.metrics.employment_separation_unexpected_response.values.fails = 1;
  assert.throws(() => validate(performance), /unexpected response.*expected_iterations|governed outcome observations/i);
});
