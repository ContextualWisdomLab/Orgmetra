...
# session role/context so it can prove forced RLS with a NOBYPASSRLS role.
...
GRANT SELECT ON candidate_application_record_version TO orgmetra_candidate_application_reader;
GRANT SELECT ON candidate_application_stage_record TO orgmetra_candidate_application_reader;
-- INSERT proves the anchor policy WITH CHECK path, not only read visibility.
...
BEGIN
    ...
    SELECT count(*) INTO version_count FROM candidate_application_record_version;
    SELECT count(*) INTO stage_count FROM candidate_application_stage_record;

    IF application_count <> 0 OR version_count <> 0 OR stage_count <> 0 THEN
        RAISE EXCEPTION
            'missing tenant context exposed candidate application history: applications=%, versions=%, stages=%',
            application_count,
            version_count,
            stage_count;
    END IF;
...
BEGIN
    ...
    IF application_count < 1 OR version_count < 1 OR stage_count < 1 THEN
        RAISE EXCEPTION
            'tenant alpha did not observe its candidate application evidence: applications=%, versions=%, stages=%',
            application_count,
            version_count,
            stage_count;
    END IF;
...
BEGIN
    ...
    IF application_count <> 0 OR version_count <> 0 OR stage_count <> 0 THEN
        RAISE EXCEPTION
            'tenant beta observed tenant-alpha candidate application history: applications=%, versions=%, stages=%',
            application_count,
            version_count,
            stage_count;
    END IF;
...
# alpha, must be rejected when it tries to persist a beta-tenant anchor. The beta
# tenant/candidate fixture exists, so policy enforcement is the first intended
# security boundary rather than a missing-reference failure.
set +e
