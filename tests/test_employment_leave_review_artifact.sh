...
package_root="${repository_root}/packages/employment-leave-review"
requirements_path="${repository_root}/.github/requirements/foundation-test.txt"
retired_workflow="${repository_root}/.github/workflows/employment-leave-review-quality.yml"
build_requirement="/tmp/orgmetra-employment-leave-review-build.txt"
build_tree="/tmp/orgmetra-employment-leave-review-build-tree"
dist_dir="/tmp/orgmetra-employment-leave-review-dist"
venv_dir="/tmp/orgmetra-employment-leave-review-venv"
install_requirement="/tmp/orgmetra-employment-leave-review-install.txt"

if [[ -e "${retired_workflow}" ]]; then
  printf 'Retired Employment Leave Review leaf workflow must not exist.\n' >&2
  exit 1
fi
...
if [[ "${#wheels[@]}" -ne 1 ]]; then
  printf 'Employment Leave Review build must produce exactly one wheel.\n' >&2
  exit 1
fi
wheel_path="${wheels[0]}"
...
printf 'orgmetra-employment-leave-review[test] @ file://%s --hash=sha256:%s\n' "${wheel_path}" "${wheel_sha}" > "${install_requirement}"
...
import pytest
import pytest_cov
import orgmetra_employment_leave_review

venv_root = Path("/tmp/orgmetra-employment-leave-review-venv").resolve()
module_path = Path(orgmetra_employment_leave_review.__file__).resolve()
...
if "test" not in (metadata("orgmetra-employment-leave-review").get_all("Provides-Extra") or []):
    raise SystemExit("built distribution does not expose the reviewed test extra")
PY

cd /tmp
COVERAGE_FILE=/tmp/orgmetra-employment-leave-review.coverage \
