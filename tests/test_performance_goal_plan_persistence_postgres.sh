...
for migration in \
  ...
  if [[ ! -f "${migration}" ]]; then
    echo "required performance-goal persistence migration is missing: ${migration}" >&2
    exit 1
  fi
...
JOB_VERSION_ID="10000000-0000-7000-8000-000000000032"
...
INSERT INTO job_profile_version (
  ...
  '${TENANT_ID}', '${JOB_VERSION_ID}', '${JOB_ID}',
...
WITH event_payload AS (
  SELECT jsonb_build_object(
    'specversion','1.0',
    'id','${AUDIT_ID}',
    'source','urn:orgmetra:performance_goal',
    'type','orgmetra.performance_goal.plan_persisted',
    'subject','performance_goal_plan:${PLAN_RECORD_ID}',
    'time','${ACTIVATED_AT}',
    'datacontenttype','application/json',
    'orgmetratenant','${TENANT_ID}',
    'orgmetraactor','${ACTOR_REFERENCE}',
    'orgmetrapurpose','performance_goal_plan_persistence',
    'orgmetrareason','activated_goal_plan_record',
    'orgmetraevidence','${ACTIVATION_DIGEST}',
...
INSERT INTO outbox_delivery_record (
  tenant_record_id, outbox_delivery_record_id, audit_event_record_id,
...
INSERT INTO performance_goal_plan_record (
  ...
  employment_record_id, job_profile_id, performance_cycle_reference,
  created_by_actor_reference
) VALUES (
  '${TENANT_ID}', '${PLAN_RECORD_ID}', '${PLAN_REFERENCE}',
  '${EMPLOYMENT_ID}', '${JOB_ID}', '${CYCLE_REFERENCE}', '${ACTOR_REFERENCE}'
);
...
INSERT INTO performance_goal_plan_version (
  ...
  '${TENANT_ID}', '${PLAN_VERSION_ID}', '${PLAN_RECORD_ID}',
  '${GOAL_DIGEST}', '${MEASUREMENT_DIGEST}', 3,
  'quarterly_check_in', '${PLAN_DIGEST}', '${ACTIVATION_REFERENCE}',
  '${ACTIVATION_DIGEST}', '${AUTHORITY_REFERENCE}',
  '${AUTHORITY_DIGEST}', '${ACTOR_REFERENCE}', '${APPROVED_AT}', '${ACTIVATED_AT}',
  DATE '2026-09-01', DATE '2026-12-31', '${AUDIT_ID}'
);
SQL

persisted="$(with_tenant "${TENANT_ID}" "${DATABASE_URL}" -Atqc "
SELECT persistence_state || '|' || rating_authority_state || '|' ||
       employment_decision_authority_state || '|' || feedback_cadence_code || '|' || goal_count
FROM performance_goal_plan_version
WHERE performance_goal_plan_version_id='${PLAN_VERSION_ID}'::uuid;
")"
if [[ "${persisted}" != "authoritatively_persisted|not_authorized_for_performance_rating|not_authorized_for_employment_decision|quarterly_check_in|3" ]]; then
  echo "performance-goal plan persisted unsafe or incomplete state: ${persisted}" >&2
  exit 1
fi

expect_failure \
  "goal-plan version accepted caller-backdated system time" \
  ...
  "INSERT INTO performance_goal_plan_version (
     ...
     activation_evidence_digest_sha256, authority_evidence_reference,
     authority_evidence_digest_sha256, approving_actor_reference, approved_at, activated_at,
     effective_from, effective_to, audit_event_record_id, recorded_from
   ) VALUES (
     '${TENANT_ID}', '10000000-0000-7000-8000-000000000053', '${PLAN_RECORD_ID}',
     '${GOAL_DIGEST}', '${MEASUREMENT_DIGEST}', 3, 'quarterly_check_in', '${PLAN_DIGEST}',
     'performance_goal_activation:00000000-0000-4000-8000-000000000054', '${ACTIVATION_DIGEST}',
     '${AUTHORITY_REFERENCE}', '${AUTHORITY_DIGEST}', '${ACTOR_REFERENCE}',
     '${APPROVED_AT}', '${ACTIVATED_AT}', DATE '2026-09-01', DATE '2026-12-31', '${AUDIT_ID}',
     TIMESTAMPTZ '2000-01-01 00:00:00+00'
   );"

expect_failure \
  "goal-plan version accepted mismatched activation audit evidence" \
  "audit evidence does not match" \
  "INSERT INTO performance_goal_plan_version (
     tenant_record_id, performance_goal_plan_version_id, performance_goal_plan_record_id,
     goal_set_digest_sha256, measurement_definition_digest_sha256, goal_count,
     feedback_cadence_code, plan_evidence_digest_sha256, activation_reference,
     ...
     '${TENANT_ID}', '10000000-0000-7000-8000-000000000055', '${PLAN_RECORD_ID}',
     '${GOAL_DIGEST}', '${MEASUREMENT_DIGEST}', 3, 'quarterly_check_in', '${PLAN_DIGEST}',
...
expect_failure \
  "goal-plan version accepted non-staffable Employment interval" \
  ...
  "INSERT INTO performance_goal_plan_version (
     ...
     '${TENANT_ID}', '10000000-0000-7000-8000-000000000057', '${PLAN_RECORD_ID}',
     '${GOAL_DIGEST}', '${MEASUREMENT_DIGEST}', 3, 'quarterly_check_in', '${PLAN_DIGEST}',
     'performance_goal_activation:00000000-0000-4000-8000-000000000058', '${ACTIVATION_DIGEST}',
     '${AUTHORITY_REFERENCE}', '${AUTHORITY_DIGEST}', '${ACTOR_REFERENCE}',
     '${APPROVED_AT}', '${ACTIVATED_AT}', DATE '2027-02-01', DATE '2027-03-01', '${AUDIT_ID}'
   );"

expect_failure \
  "goal-plan version accepted Job interval without authoritative Job coverage" \
  "Job coverage" \
  "INSERT INTO performance_goal_plan_version (
     tenant_record_id, performance_goal_plan_version_id, performance_goal_plan_record_id,
     goal_set_digest_sha256, measurement_definition_digest_sha256, goal_count,
     ...
     '${TENANT_ID}', '10000000-0000-7000-8000-000000000059', '${PLAN_RECORD_ID}',
     '${GOAL_DIGEST}', '${MEASUREMENT_DIGEST}', 3, 'quarterly_check_in', '${PLAN_DIGEST}',
     'performance_goal_activation:00000000-0000-4000-8000-000000000060', '${ACTIVATION_DIGEST}',
     '${AUTHORITY_REFERENCE}', '${AUTHORITY_DIGEST}', '${ACTOR_REFERENCE}',
     '${APPROVED_AT}', '${ACTIVATED_AT}', DATE '2027-01-01', DATE '2027-01-15', '${AUDIT_ID}'
   );"

expect_failure \
  "goal-plan evidence was rewriteable" \
  "immutable" \
...
UPDATE performance_goal_plan_version
SET recorded_to = pg_catalog.transaction_timestamp()
WHERE performance_goal_plan_version_id='${PLAN_VERSION_ID}'::uuid;"

expect_failure \
  "goal-plan history was deletable" \
  "immutable" \
  "DELETE FROM performance_goal_plan_version WHERE performance_goal_plan_record_id='${PLAN_RECORD_ID}'::uuid;"
expect_failure \
  "goal-plan history could be truncated" \
  "cannot be truncated" \
  "TRUNCATE performance_goal_plan_version;"
...
for forbidden_column in goal_text performance_rating assessment_score compensation_value candidate_reference free_form_note prompt_text model_output; do
  count="$(psql "${DATABASE_URL}" -Atqc "
    ...
    WHERE table_schema='public'
      AND table_name IN ('performance_goal_plan_record','performance_goal_plan_version')
      AND column_name='${forbidden_column}';")"
  if [[ "${count}" != "0" ]]; then
    echo "goal-plan persistence introduced prohibited sensitive column: ${forbidden_column}" >&2
    exit 1
  fi
done
...
FROM pg_class
WHERE relname IN ('performance_goal_plan_record','performance_goal_plan_version');")"
if [[ "${rls_state}" != "performance_goal_plan_record:true:true,performance_goal_plan_version:true:true" ]]; then
  echo "performance-goal RLS is not enabled and forced: ${rls_state}" >&2
  exit 1
fi
...
BEGIN;
SET LOCAL ROLE performance_goal_reader;
SET LOCAL orgmetra.tenant_record_id='${TENANT_ID}';
SELECT count(*) FROM performance_goal_plan_record;
COMMIT;" | tail -n 1)"
if [[ "${visible}" != "1" ]]; then
  echo "tenant reader could not see own performance-goal anchor: ${visible}" >&2
  exit 1
fi
...
COMMIT;" | tail -n 1)"
if [[ "${hidden}" != "0" ]]; then
  echo "foreign tenant could see performance-goal anchors: ${hidden}" >&2
  exit 1
fi
