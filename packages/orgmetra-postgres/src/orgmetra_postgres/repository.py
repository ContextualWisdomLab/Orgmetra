...
class PostgresPeopleRepository:
    ...
    The host is responsible for authenticating the actor and authorizing the
    purpose before constructing :class:`PurposeContext`. Every operation binds
    the tenant to a transaction-local PostgreSQL setting consumed by row-level
    security policies.
    """
...
def _normalize_code(value: str, field_name: str) -> str:
    """Normalize a bounded machine-readable code."""
    ...
    if not all(character.islower() or character.isdigit() or character == "_" for character in normalized):
        raise ValueError(
            f"{field_name} must use lower-case letters, digits, and underscores"
        )
    return normalized
