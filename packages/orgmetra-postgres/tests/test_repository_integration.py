...
def application_dsn() -> Iterator[str]:
    """Create a fresh migrated database surface and a non-bypass application role."""
    ...
    migration_paths = sorted((repository_root / "database" / "migrations").glob("*.sql"))
        ...
        connection.execute(
            ...
            BEGIN
                IF EXISTS (
                    ...
                    DROP OWNED BY orgmetra_test_app;
                    DROP ROLE orgmetra_test_app;
                END IF;
            END
            $$
            """
    ...
    with psycopg.connect(application_dsn) as connection:
        with connection.transaction():
            ...
            with pytest.raises(
                ...
                connection.execute(
                    "UPDATE audit_event SET purpose_code = 'changed' WHERE resource_record_id = %s",
                    (person_id,),
                )
