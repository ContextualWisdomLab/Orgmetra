...
- Read every trust-bearing scalar once into a local canonical-export snapshot, validate exact built-in runtime types on that same snapshot, and serialize only those captured values; this closes the remaining validation-to-emission race while the issuance digest continues to detect semantic mutation.
