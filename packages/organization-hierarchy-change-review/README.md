...
The top-level issuance semantic validator is bound at the same packet-runtime construction boundary. Replacing the later module-level `_validate_issuance_snapshot` name therefore cannot turn mandatory requester/reviewer separation, controlled-purpose/reason, PII minimization, temporal, identifier, or review-state checks into a no-op before the creation seal is written. This is a narrow ordinary module-binding defense: it does not freeze every transitive helper or claim protection against deliberate function-global/closure introspection or arbitrary same-process/native execution.

Snapshot selection and canonical-byte construction use the same boundary. The packet runtime captures `_snapshot`, `_payload_from_snapshot`, and `_canonical_payload_json` before packet construction. Issuance therefore validates, canonicalizes, seals, stores exact field state, and derives the live-reference key from the same runtime-selected snapshot even if a later consumer reassigns those module names. This prevents an ordinary module mutation from validating one object or snapshot while sealing different evidence. It does not freeze transitive helper internals or create a general Python sandbox.
