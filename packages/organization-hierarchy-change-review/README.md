...
`recorded_at` has an additional nested-runtime and identity check before export. An exact built-in `datetime` can still contain caller-defined `tzinfo`; Python datetime comparison may invoke that object's `utcoffset()` behavior. Python also defines equality for aware datetimes by represented instant, so two distinct built-in datetime objects such as `16:00+09:00` and `07:00+00:00` compare equal even though the retained representation changed. Issuance validates the captured timestamp as an exact built-in datetime with a built-in fixed-offset timezone and stores that immutable object in closure-private issuance state. Export therefore first rejects a non-built-in nested timezone and then requires the retained datetime to be the same immutable object captured at issuance, without entering instant-based datetime equality. A custom-`tzinfo` substitution is rejected without executing the nested timezone callback, and a same-instant fixed-offset replacement is also rejected. This remains process-local defense in depth, not a Python sandbox.
