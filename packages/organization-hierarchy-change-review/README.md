...
Caller-defined packet classes are rejected when subclass creation is attempted, before a subclass can replace `__post_init__`, `__getattribute__`, or another validation/emission hook. Caller-defined subclasses of trust-bearing strings, integers, dates, or datetimes are likewise rejected before equality, ordering, membership, parsing, or canonical emission can depend on caller polymorphism. Canonical evidence is deterministic and the routine `repr` is redacted.
