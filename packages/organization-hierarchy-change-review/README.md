...
Caller-defined packet classes are rejected when subclass creation is attempted, before a subclass can replace `__post_init__`, `__getattribute__`, or another validation/emission hook. Caller-defined subclasses of trust-bearing strings, integers, dates, or datetimes are likewise rejected at issuance. Canonical export then revalidates every trust-bearing scalar's exact built-in runtime type before serialization, so low-level post-issuance replacement with a `str` or `int` subclass is rejected even when its JSON value and SHA-256 bytes would otherwise remain identical. Canonical evidence is deterministic and the routine `repr` is redacted.
