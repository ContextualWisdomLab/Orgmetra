...
The packet requires opaque canonical non-sentinel UUIDv4-backed references for the Person, Employment, current Assignment, current Job and Position, proposed Job and Position, current-scope snapshot, reviewed workforce-allocation plan, exact workforce-allocation policy, worker-impact assessment, communication plan, requester, and reviewer. UUIDv1 and every other UUID version are rejected so timestamp/node correlation metadata cannot enter an otherwise opaque trust-reference field. Each trust-bearing evidence artifact is paired with a lowercase SHA-256 digest. A bounded positive `evidence_version` is part of canonical evidence so actor/purpose/reason evidence from different review-contract revisions cannot silently collide.
