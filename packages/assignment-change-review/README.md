...
The packet requires a canonical non-sentinel UUIDv4 `tenant_record_id` plus opaque canonical non-sentinel UUIDv4-backed references for the Person, Employment, current Assignment, current Job and Position, proposed Job and Position, current-scope snapshot, reviewed workforce-allocation plan, exact workforce-allocation policy, worker-impact assessment, communication plan, requester, and reviewer. UUIDv1 and every other UUID version are rejected for tenant and reference identities so timestamp/node correlation metadata cannot enter values presented as opaque governance identity. Each trust-bearing evidence artifact is paired with a lowercase SHA-256 digest. A bounded positive `evidence_version` is part of canonical evidence so actor/purpose/reason evidence from different review-contract revisions cannot silently collide.
