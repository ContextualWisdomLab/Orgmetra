"""Executable documentation-completeness contract for candidate evidence intake."""
...
SOURCE_ROOT = PACKAGE_ROOT / "src" / "orgmetra_candidate_evidence"
TEST_ROOT = PACKAGE_ROOT / "tests"


def _python_files() -> tuple[Path, ...]:
    """Return every owned Python source and test file in deterministic order."""
    return tuple(sorted((*SOURCE_ROOT.glob("*.py"), *TEST_ROOT.glob("*.py"))))


def test_owned_python_modules_and_callables_are_documented() -> None:
    """Require beginner-readable docstrings on all owned modules, classes, and callables."""
    missing: list[str] = []
