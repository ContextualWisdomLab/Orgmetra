...
After successful construction, the packet binds its exact canonical JSON to a process-local HMAC issuance seal and registers the issued object identity for the lifetime of that packet. `canonical_json()` and `sha256_digest()` require that exact live issued identity, then recompute and verify the seal before returning evidence, so post-issuance mutation through low-level Python attribute operations fails closed even when the replacement value would otherwise pass field validation. Re-running `__post_init__()` cannot reissue or legitimize changed evidence even after seal deletion. Constructor eligibility is separately registered before dataclass initialization and consumed exactly once after successful issuance, so a slot-for-slot clone made with `object.__new__`, even when it copies valid seal bytes, cannot export evidence or mint a replacement issuance. Because a copied object would otherwise obtain a fresh runtime identity carrying copied seal bytes, shallow and deep copy preserve the same immutable packet identity, while pickle serialization is rejected rather than emitting evidence whose process-local issuance identity cannot be carried across the boundary. This is **in-process mutation defense-in-depth only**: the random runtime seal and identity registries are not a durable digital signature, external attestation, persistence mechanism, or substitute for immutable audit/outbox storage. Hosts that persist or transmit evidence must preserve the canonical JSON/digest and independently enforce their durable provenance controls.
