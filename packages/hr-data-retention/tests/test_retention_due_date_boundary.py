"""Boundary regression for the reviewed retention due date."""
...
def test_review_exactly_on_due_date_still_requires_retention() -> None:
    """Treat the reviewed due date as retained-through, not immediately disposable."""
    packet = HrDataRetentionReviewPacket(
    ...
    assert packet.disposition_authorization_state == "not_authorized_to_delete"
    assert "Retain the record through the reviewed retention due date" in packet.next_action
