...
- Bind plan issuance to exact live governed-constructor provenance before `__post_init__()` may register creation evidence, so an `object.__new__` clone cannot copy valid plan fields and mint a fresh issued identity by manually invoking initialization.
