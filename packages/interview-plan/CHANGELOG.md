...
- Arm plan-construction provenance only during the metaclass-mediated full `StructuredInterviewPlan(...)` call and consume it in `__post_init__()`, so neither `object.__new__` nor direct `StructuredInterviewPlan.__new__(StructuredInterviewPlan)` allocation can copy otherwise valid fields and mint activation-ready issuance evidence by manually invoking initialization.
