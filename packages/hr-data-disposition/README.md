...
Construction seals the canonical request digest in a process-local identity-keyed weak registry outside packet-writable state. Canonicalization revalidates live fields, computes one verified document snapshot, and returns that same snapshot so a concurrent low-level mutation cannot replace the document after the seal check; a different valid policy, actor, date, or upstream-evidence value is rejected instead of creating a second audit fact. A caller cannot replace an in-object seal. Durable append-only audit, replay/idempotency, and execution authorization remain separate authoritative responsibilities.
