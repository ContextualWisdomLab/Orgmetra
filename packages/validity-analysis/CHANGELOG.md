...
- Add typed `NonresponseAdjustmentReceipt` evidence with explicit disposition treatment and typed `CalibrationAdjustmentReceipt` evidence bound to purpose-limited auxiliary projections, explicit scientific-use purpose, released owner-contract coordinates, authorization-receipt digest, authoritative benchmark receipts, constraints, and explicit convergence/fallback state.
...
- Keep durable owner-side typed-receipt and auxiliary-authorization resolution plus released auxiliary evidence exchange explicitly incomplete under #407.
