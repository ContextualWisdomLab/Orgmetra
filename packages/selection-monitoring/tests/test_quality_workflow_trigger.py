...
_ARTIFACT_CONTRACT = _REPOSITORY_ROOT / "tests/test_selection_monitoring_artifact.sh"
...
def test_foundation_owns_selection_monitoring_quality_contract() -> None:
    """Require canonical Foundation CI to execute the package's exact coverage contract."""
    ...
    assert 'selection_monitoring_contract="${repository_root}/tests/test_selection_monitoring_artifact.sh"' in hygiene
    assert 'bash "${selection_monitoring_contract}"' in hygiene
