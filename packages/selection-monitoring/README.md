...
A frozen dataclass is not by itself issuance evidence because low-level Python mutation can still rewrite otherwise valid values. Each live issued plan is therefore bound to its exact construction-time canonical JSON by a process-local HMAC seal stored outside packet-writable slots. Seal registration is single-use per live object identity: explicitly re-running `__post_init__()` cannot replace an existing seal after a low-level valid-value rewrite. Canonical export captures every serialized trust-bearing field exactly once, exact-type validates that captured snapshot, and performs date/timestamp rendering plus JSON serialization only from that same snapshot. This prevents a subtype present at capture from reaching caller-controlled rendering and prevents a concurrent mutation after validation from substituting a different live value into the same export. `canonical_json()` then verifies those exact canonical snapshot bytes against the external issuance seal and returns the verified snapshot without rereading the object. A valid-value rewrite, representation-preserving runtime alias, validation/rendering checked-versus-used mutation, attempted seal renewal, or missing process-local issuance evidence fails closed. This mechanism is defense-in-depth for in-process misuse only; durable cross-process uniqueness, purpose authorization, and immutable audit/outbox remain responsibilities of the authoritative host or persistence boundary.
