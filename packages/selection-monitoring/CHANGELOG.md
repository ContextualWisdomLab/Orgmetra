...
- Make `Selection Monitoring Quality` retrigger when shared repository Python/test/clean-checkout configuration changes (`.gitignore`, `.python-version`, root/shared `conftest.py`, root `pyproject.toml`, `pytest.ini`, `setup.cfg`, or `tox.ini`) and enforce that trigger surface with an executable regression, preventing stale package-quality evidence after shared tooling changes.
