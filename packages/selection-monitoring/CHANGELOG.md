...
- Revalidate every canonical-export trust field against its exact built-in runtime type before date/timestamp rendering, JSON serialization, or HMAC comparison, so a post-issuance behavior-bearing subtype cannot retain authority merely by preserving the same canonical bytes.
