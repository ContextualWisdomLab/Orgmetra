"""Corroborate append-only nonresponse-adjustment correction authority.

The ordinary nonresponse projection proves which disposition-aware adjustment
receipt produced a released point-weight artifact. This boundary proves when
that immutable receipt remained authoritative and which released successor
ended its half-open authority interval. Successor chronology stays owner-resolved
instead of becoming a caller-selected lookup coordinate.
"""
...
class NonresponseAdjustmentSupersessionAuthorityNotFound(LookupError):
    """Indicate that no released owner evidence corroborates the receipt."""


class NonresponseAdjustmentSupersessionAuthorityIntegrityError(RuntimeError):
    """Indicate that released correction evidence cannot authorize use."""


class NonresponseAdjustmentSupersessionAuthorityRecord(tuple):
    """Immutable owner projection for one nonresponse receipt authority interval."""
    ...
    def __new__(
        ...
        *,
        ...
        successor_nonresponse_receipt_reference: str | None = None,
        successor_nonresponse_receipt_digest: str | None = None,
        ...
        if owner_release > release_instant:
            raise ValueError(
                "owner contract must be released no later than nonresponse receipt."
            )
        ...
        if all(value is None for value in successor_values):
            ...
            raise ValueError(
                "nonresponse supersession requires cutover and complete released successor coordinates."
            )
        else:
            ...
            if cutover <= release_instant:
                raise ValueError("superseded_at must be later than nonresponse receipt release.")
            if successor_ref == receipt_ref:
                raise ValueError("successor nonresponse receipt must have a new reference.")
            if successor_digest == receipt_digest:
                raise ValueError("successor nonresponse receipt must identify new evidence.")
            if successor_version != 1:
                raise ValueError("successor_evidence_version must remain 1.")
            if successor_release <= release_instant:
                raise ValueError(
                    "successor nonresponse receipt must be released after its predecessor."
                )
            if successor_release != cutover:
                raise ValueError(
                    "successor nonresponse receipt must be released exactly at supersession."
                )

        current_fields: tuple[tuple[str, object], ...] = (
            ("evidence_version", version),
            ("nonresponse_receipt_digest", receipt_digest),
            ("nonresponse_receipt_reference", receipt_ref),
        ...
        if cutover is None:
            ...
            successor_fields = (
                ...
                ("successor_nonresponse_receipt_reference", successor_ref),
    ...
    def released_at(self) -> datetime:
        """Return when this nonresponse receipt became released authority."""
        return self[3]
...
class NonresponseAdjustmentSupersessionAuthorityView:
    ...
    def __new__(
        ...
        *,
        ...
        raise TypeError(
            "NonresponseAdjustmentSupersessionAuthorityView is issued only by "
    ...
    def __setattr__(self, name: str, value: object) -> None:
        ...
        raise AttributeError(
            "NonresponseAdjustmentSupersessionAuthorityView is immutable."
        )
...
@runtime_checkable
class NonresponseAdjustmentSupersessionAuthorityReadPort(Protocol):
    """Owner read contract for one released nonresponse correction state."""

    def read_nonresponse_adjustment_supersession_authority(
        self,
        *,
        tenant_record_id: UUID,
        validity_study_id: UUID,
        ...
        owner_contract_version: int,
        owner_contract_digest: str,
    ) -> NonresponseAdjustmentSupersessionAuthorityRecord | None:
        """Return matching released nonresponse supersession evidence or ``None``."""
        ...


_PROTOCOL_READ_CAPABILITY = getattr_static(
    NonresponseAdjustmentSupersessionAuthorityReadPort,
    "read_nonresponse_adjustment_supersession_authority",
)
...
def resolve_nonresponse_adjustment_supersession_authority(
    *,
    ...
    read_capability = getattr_static(
        type(read_port), "read_nonresponse_adjustment_supersession_authority", None
    )
    if (
    ...
    if persisted is None:
        raise NonresponseAdjustmentSupersessionAuthorityNotFound(str(study_id))
    if type(persisted) is not NonresponseAdjustmentSupersessionAuthorityRecord:
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "owner port returned non-canonical nonresponse supersession evidence"
        )
    ...
    try:
        ...
        record = NonresponseAdjustmentSupersessionAuthorityRecord(
            ...
            successor_nonresponse_receipt_reference=(
                ...
                else successor_values["successor_nonresponse_receipt_reference"]
            ),
            successor_nonresponse_receipt_digest=(
                None
                if successor_values is None
                else successor_values["successor_nonresponse_receipt_digest"]
            ),
            successor_evidence_version=(
                None
                if successor_values is None
        ...
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "owner port returned malformed nonresponse supersession evidence"
        ) from exc
    if record != persisted:
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "owner port returned non-canonical nonresponse supersession evidence"
    ...
    if (
        ...
        or record_values["nonresponse_receipt_reference"] != receipt_ref
        or record_values["nonresponse_receipt_digest"] != receipt_digest
        ...
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "released nonresponse supersession authority does not match requested coordinates"
        )
    if use_instant < record.released_at:
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "nonresponse receipt must be released before scientific use"
        )
    if record.superseded_at is not None and use_instant >= record.superseded_at:
        raise NonresponseAdjustmentSupersessionAuthorityIntegrityError(
            "nonresponse receipt is superseded for this scientific-use instant"
        )
