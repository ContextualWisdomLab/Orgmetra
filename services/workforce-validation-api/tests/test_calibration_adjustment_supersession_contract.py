...
class _ReadPort:
    """Return configured correction evidence and retain exact lookup coordinates."""
...
class _ProtocolOnly(CalibrationAdjustmentSupersessionAuthorityReadPort):
    """Inherit only the Protocol placeholder, not a concrete owner capability."""


class _DescriptorReadPort:
    """Expose a descriptor that must be rejected without execution."""

    @property
    def read_calibration_adjustment_supersession_authority(self) -> object:
...
def _resolve(*, read_port: object, used_at: datetime, **overrides: object):
    ...
    values.update(overrides)
    return resolve_calibration_adjustment_supersession_authority(**values)
...
def test_successor_edge_requires_complete_atomic_released_coordinates() -> None:
    ...
    with pytest.raises(ValueError, match="later than calibration receipt release"):
...
def test_chronology_requires_released_owner_and_timezone_aware_instants() -> None:
    ...
    with pytest.raises(ValueError):
        _record(released_at=datetime(2026, 9, 16, 13, 0))
    with pytest.raises(ValueError):
        _record(superseded_at=datetime(2026, 9, 17, 13, 0))
    with pytest.raises(ValueError):
        _record(successor_released_at=datetime(2026, 9, 17, 13, 0))
...
def test_historical_use_is_allowed_but_cutover_use_fails_closed() -> None:
    ...
    assert isinstance(port, CalibrationAdjustmentSupersessionAuthorityReadPort)
    assert port.calls == [
        {
            "tenant_record_id": TENANT,
            "validity_study_id": STUDY,
...
def test_missing_noncanonical_and_pre_release_owner_evidence_fail_closed() -> None:
    with pytest.raises(CalibrationAdjustmentSupersessionAuthorityNotFound):
        _resolve(read_port=_ReadPort(None), used_at=CUTOVER - timedelta(seconds=1))
    with pytest.raises(CalibrationAdjustmentSupersessionAuthorityIntegrityError):
        _resolve(read_port=_ReadPort(object()), used_at=CUTOVER - timedelta(seconds=1))
    with pytest.raises(
        CalibrationAdjustmentSupersessionAuthorityIntegrityError,
        match="released before scientific use",
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("used_at", datetime(2026, 9, 17, 12, 0), ValueError),
