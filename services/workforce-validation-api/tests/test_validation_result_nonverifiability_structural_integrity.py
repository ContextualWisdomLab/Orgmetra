"""Regression coverage for canonical non-verifiability owner evidence structure."""
...
class _ReadPort:
    ...
    def read_validation_result_nonverifiability(self, **_: object) -> object:
        return self.result


def _record() -> ValidationResultNonVerifiabilityRecord:
...
def test_owner_port_cannot_append_hidden_tuple_fields_to_exact_record_type() -> None:
    valid = _record()
...
def test_malformed_exact_record_type_maps_to_integrity_error() -> None:
    valid = _record()
    forged = tuple.__new__(ValidationResultNonVerifiabilityRecord, tuple(valid)[:-1])
