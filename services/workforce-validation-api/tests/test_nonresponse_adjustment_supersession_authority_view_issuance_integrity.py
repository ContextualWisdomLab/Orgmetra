"""Regression contract for nonresponse-supersession view issuance integrity."""
...
def test_low_level_tuple_construction_cannot_issue_nonresponse_supersession_view() -> None:
...
def test_unsealed_object_allocation_cannot_expose_nonresponse_supersession_view() -> None:
    """Require the resolver seal before raw exact-runtime objects expose state."""
    unsealed = object.__new__(NonresponseAdjustmentSupersessionAuthorityView)

    for attribute_name in ("tenant_record_id", "validity_study_id", "fields"):
        with pytest.raises(
            NonresponseAdjustmentSupersessionAuthorityIntegrityError,
            match="was not issued by resolve_nonresponse_adjustment_supersession_authority",
        ):
            getattr(unsealed, attribute_name)


def test_wrong_issuance_marker_cannot_expose_nonresponse_supersession_view() -> None:
    """Reject marker-shaped objects that did not originate from the resolver."""
    forged_view = object.__new__(NonresponseAdjustmentSupersessionAuthorityView)
...
def test_raw_nonresponse_supersession_view_rejects_mutation_and_deletion() -> None:
    """Keep projection state immutable after raw exact-runtime allocation."""
    raw_view = object.__new__(NonresponseAdjustmentSupersessionAuthorityView)
