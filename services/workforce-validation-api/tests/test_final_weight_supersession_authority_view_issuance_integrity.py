"""Regression contract for final-weight supersession view issuance integrity."""
...
def test_unsealed_object_allocation_cannot_expose_supersession_view() -> None:
    ...
    unsealed = object.__new__(FinalWeightSupersessionAuthorityView)

    for attribute_name in ("tenant_record_id", "validity_study_id", "fields"):
        with pytest.raises(
            FinalWeightSupersessionAuthorityIntegrityError,
            match="was not issued by resolve_final_weight_supersession_authority",
        ):
...
def test_wrong_issuance_marker_cannot_expose_supersession_view() -> None:
    ...
    forged_view = object.__new__(FinalWeightSupersessionAuthorityView)
...
def test_raw_supersession_view_rejects_mutation_and_deletion() -> None:
    ...
    raw_view = object.__new__(FinalWeightSupersessionAuthorityView)
