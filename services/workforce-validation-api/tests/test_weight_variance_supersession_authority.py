"""Append-only correction contract for point-weight/variance compatibility authority."""
...
class _ReadPort:
    ...
    def read_weight_variance_supersession_authority(self, **coordinates: object) -> object:
        self.calls.append(dict(coordinates))
        return self.result


class _ProtocolOnly(WeightVarianceSupersessionAuthorityReadPort):
    pass


class _DescriptorReadPort:
    @property
    def read_weight_variance_supersession_authority(self) -> object:
...
def _record(**overrides: object) -> WeightVarianceSupersessionAuthorityRecord:
    ...
    return WeightVarianceSupersessionAuthorityRecord(**values)
...
def _resolve(*, read_port: object, used_at: datetime, **overrides: object):
    ...
    values.update(overrides)
    return resolve_weight_variance_supersession_authority(**values)
...
def test_successor_edge_requires_complete_atomic_released_coordinates() -> None:
    ...
    with pytest.raises(ValueError, match="new reference"):
        _record(successor_authority_reference=AUTHORITY)
...
def test_chronology_requires_released_owner_and_timezone_aware_instants() -> None:
    ...
    with pytest.raises(ValueError):
        _record(owner_contract_released_at=datetime(2026, 9, 17, 0, 30))
    with pytest.raises(ValueError):
        _record(released_at=datetime(2026, 9, 17, 1, 0))
    with pytest.raises(ValueError):
        _record(superseded_at=datetime(2026, 9, 18, 1, 0))
    with pytest.raises(ValueError):
        _record(successor_released_at=datetime(2026, 9, 18, 1, 0))
...
def test_historical_use_is_allowed_but_cutover_use_fails_closed() -> None:
    ...
    assert ("authority_reference", AUTHORITY) in view.fields
    ...
    with pytest.raises(WeightVarianceSupersessionAuthorityIntegrityError, match="superseded"):
...
def test_open_interval_without_successor_remains_current() -> None:
    record = _record(
        superseded_at=None,
        successor_authority_reference=None,
...
def test_missing_noncanonical_and_pre_release_evidence_fail_closed() -> None:
    with pytest.raises(WeightVarianceSupersessionAuthorityNotFound):
    ...
    with pytest.raises(WeightVarianceSupersessionAuthorityIntegrityError, match="released before scientific use"):
...
def test_owner_evidence_must_match_every_requested_coordinate(record_overrides: dict[str, object]) -> None:
    if "authority_reference" in record_overrides:
        record_overrides = {
            **record_overrides,
            "successor_authority_reference": AUTHORITY,
        }
    with pytest.raises(WeightVarianceSupersessionAuthorityIntegrityError):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("used_at", datetime(2026, 9, 18, 0, 30), ValueError),
