"""Regression contract for calibration-support view issuance integrity."""
...
def test_unsealed_object_allocation_cannot_expose_support_view() -> None:
    """Require the resolver seal before raw exact-runtime objects expose state."""
    unsealed = object.__new__(CalibrationSupportAuthorityView)

    for attribute_name in ("tenant_record_id", "validity_study_id", "fields"):
        with pytest.raises(
            CalibrationSupportAuthorityIntegrityError,
            match="was not issued by resolve_calibration_support_authority",
        ):
            getattr(unsealed, attribute_name)


def test_wrong_issuance_marker_cannot_expose_support_view() -> None:
...
def test_raw_support_view_rejects_mutation_and_deletion() -> None:
    """Keep projection state immutable after raw exact-runtime allocation."""
    raw_view = object.__new__(CalibrationSupportAuthorityView)
