"""Hostile edges for append-only nonresponse-adjustment correction authority."""
...
OTHER_RECEIPT = "nonresponse_adjustment_receipt:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"
...
class _ProtocolOnly(NonresponseAdjustmentSupersessionAuthorityReadPort):
...
class _DescriptorReadPort:
    ...
    @property
    def read_nonresponse_adjustment_supersession_authority(self) -> object:
...
def _policy(*, purpose_code: str = "selection_validity_analysis") -> PurposeBoundAccessPolicy:
    """Return the purpose-bound policy for nonresponse correction evidence."""
...
def _record(**overrides: object) -> NonresponseAdjustmentSupersessionAuthorityRecord:
    """Build one current nonresponse correction state."""
    values: dict[str, object] = {
        ...
        "superseded_at": None,
        "successor_nonresponse_receipt_reference": None,
        "successor_nonresponse_receipt_digest": None,
...
def _resolve(
    *, read_port: object, **overrides: object
    ...
    """Resolve current nonresponse authority with caller-known coordinates only."""
    values: dict[str, object] = {
    ...
    values.update(overrides)
    return resolve_nonresponse_adjustment_supersession_authority(**values)
...
def test_current_receipt_resolution_uses_owner_chronology_without_successor() -> None:
    ...
    assert isinstance(port, NonresponseAdjustmentSupersessionAuthorityReadPort)
...
def test_missing_or_noncanonical_owner_evidence_fails_closed() -> None:
    ...
    with pytest.raises(NonresponseAdjustmentSupersessionAuthorityNotFound):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"nonresponse_receipt_reference": OTHER_RECEIPT},
...
def test_record_rejects_invalid_chronology_schema_and_public_view() -> None:
    ...
    with pytest.raises(ValueError, match="later than nonresponse"):
        _record(
