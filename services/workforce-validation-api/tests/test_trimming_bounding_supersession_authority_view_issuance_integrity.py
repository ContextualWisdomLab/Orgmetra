"""Regression contract for trimming/bounding supersession view issuance integrity."""
    ...
    with pytest.raises(TypeError):
        tuple.__new__(
            ...
            (
                TENANT.int,
                STUDY.int,
                (("adjustment_receipt_digest", "6" * 64),),
            ),
        )
...
def test_unsealed_object_allocation_cannot_expose_supersession_view() -> None:
    ...
    unsealed = object.__new__(TrimmingBoundingSupersessionAuthorityView)

    for attribute_name in ("tenant_record_id", "validity_study_id", "fields"):
        with pytest.raises(
            TrimmingBoundingSupersessionAuthorityIntegrityError,
            match="was not issued by resolve_trimming_bounding_supersession_authority",
        ):
...
def test_wrong_issuance_marker_cannot_expose_supersession_view() -> None:
    ...
    forged_view = object.__new__(TrimmingBoundingSupersessionAuthorityView)
...
def test_raw_supersession_view_rejects_mutation_and_deletion() -> None:
    ...
    raw_view = object.__new__(TrimmingBoundingSupersessionAuthorityView)
