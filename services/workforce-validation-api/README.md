...
The governing owner-contract release instant and optional exclusive `superseded_at` are owner-resolved evidence, never caller lookup coordinates. The contract must already exist when verification is evaluated, and the ordinary resolver accepts the released negative outcome only on `[released_at, superseded_at)`. This prevents an earlier `not_verifiable` outcome from remaining apparently current after later released evidence makes the same immutable validation result corroboratable; use at or after the owner cutover fails closed while historical use before cutover remains reproducible.
