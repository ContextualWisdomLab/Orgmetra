"""Executable HTTP transport contracts for governed Employment-history reads."""
...
VERSION_A = UUID("0198a414-6000-7000-8000-000000000020")
VERSION_B = UUID("0198a414-6000-7000-8000-000000000021")
EMPLOYMENT_A = UUID("0198a414-6000-7000-8000-000000000030")
EMPLOYMENT_B = UUID("0198a414-6000-7000-8000-000000000031")
...
class FakeReadPort:
    """Return configured Employment history and capture protected reads."""

    def __init__(self, records: tuple[EmploymentHistoryRecord, ...]) -> None:
...
def history_record(
    *,
    ...
    """Build one canonical Employment-history fixture."""
...
class EmploymentHistoryHttpRouteTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.policy = PurposeBoundAccessPolicy(
            ...
            permitted_fields=frozenset(
                {"effective_from", "employment_status_code", "recorded_to"}
            ),
        )
    ...
    async def test_get_history_returns_only_authorized_fields_with_private_cache_controls(self) -> None:
        ...
        self.assertEqual(payload["resource_reference"], f"person_employment_history:{PERSON.hex}")
        self.assertEqual(
            payload["entries"],
            [{"fields": {"effective_from": "2026-01-01", "employment_status_code": "active"}}],
        )
    ...
    async def test_malformed_request_fails_before_authentication_or_protected_read(self) -> None:
        ...
        port = FakeReadPort((history_record(),))
        app = self._app(authenticator=authenticator, read_port=port)
        zero_tenant_path = f"/v1/tenants/{UUID(int=0)}/people/{PERSON}/employment-history"
        max_person_path = f"/v1/tenants/{TENANT}/people/{UUID(int=(1 << 128) - 1)}/employment-history"
        cases = (
            {"path": "/v1/tenants/not-a-uuid/people/not-a-uuid/employment-history"},
            {"path": zero_tenant_path},
            {"path": max_person_path},
            {"query": "known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&fields=effective_from"},
            {"query": b"\xff"},
            {"query": b"bogus"},
            {"query": b"purpose=employee_profile_review&fields=effective_from"},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&purpose=other&fields=effective_from"},
            {"query": b"known_at=2026-08-30T00:00:00+00:00&purpose=employee_profile_review&fields=effective_from"},
            {"query": b"known_at=2026-08-30&purpose=employee_profile_review&fields=effective_from"},
            {"query": b"known_at=not-a-time&purpose=employee_profile_review&fields=effective_from"},
            {"query": b"known_at=2026-02-30T00:00:00Z&purpose=employee_profile_review&fields=effective_from"},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=EmployeeProfileReview&fields=effective_from"},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=&fields=effective_from"},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&fields="},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&fields=EffectiveFrom"},
            {"query": b"known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&fields=effective_from,effective_from"},
        )
        for case in cases:
    ...
    async def test_wrong_path_and_method_return_transport_errors_without_authentication(self) -> None:
        ...
        wrong_paths: tuple[object, ...] = (
            "/v1/unknown",
            f"/v2/tenants/{TENANT}/people/{PERSON}/employment-history",
            f"/v1/tenants/{TENANT}/people/{PERSON}/history",
    ...
    async def test_authorization_denial_does_not_read_employment_history(self) -> None:
        port = FakeReadPort((history_record(),))
        app = self._app(read_port=port)

        status, _, payload = await self._request(
            app,
            query=b"known_at=2026-08-30T00:00:00Z&purpose=employee_profile_review&fields=employment_record_id",
        )
    ...
    async def test_integrity_conflict_returns_client_safe_error(self) -> None:
        bad_record = history_record()
        bad_record = EmploymentHistoryRecord(
            tenant_record_id=bad_record.tenant_record_id,
            person_record_id=UUID("0198a414-6000-7000-8000-000000000011"),
            employment_record_id=bad_record.employment_record_id,
            employment_record_version_id=bad_record.employment_record_version_id,
            employment_status_code=bad_record.employment_status_code,
            employment_concurrency_code=bad_record.employment_concurrency_code,
            effective_from=bad_record.effective_from,
            effective_to=bad_record.effective_to,
            recorded_from=bad_record.recorded_from,
            recorded_to=bad_record.recorded_to,
        )
        status, _, payload = await self._request(self._app(read_port=FakeReadPort((bad_record,))))

        self.assertEqual((status, payload["error_code"]), (409, "employment_history_integrity_conflict"))
        self.assertNotIn("employment_record_id", json.dumps(payload))
    ...
    async def test_errors_use_the_published_client_safe_envelope(self) -> None:
        status, _, payload = await self._request(self._app(), path="/v1/not-the-employment-history-route")
