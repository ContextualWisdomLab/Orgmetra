...
class GuardAuthenticator:
    ...
    async def authenticate(self, bearer_token: str) -> AuthenticatedPrincipal:
        ...
        del bearer_token
        self.calls += 1
        raise AssertionError("oversized query reached authentication")
...
class GuardReadPort:
    ...
    def read_worker(
        ...
        *,
        ...
        self.calls += 1
        raise AssertionError("oversized query reached protected People persistence")


class PeopleHttpQueryLimitTests(unittest.IsolatedAsyncioTestCase):
    """Prove pre-auth query parsing is bounded before allocating parser work."""
    ...
    async def test_oversized_query_is_rejected_before_parser_auth_or_read(self) -> None:
        ...
        policy = PurposeBoundAccessPolicy(
            ...
            policy_version_code="http-query-limit-v1",
        ...
        app = PeopleAsgiApp(authenticator=authenticator, policy=policy, read_port=read_port)
