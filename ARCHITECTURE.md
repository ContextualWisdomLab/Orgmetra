...
`packages/orgmetra-domain` implements framework-independent invariants for effective/system time, identity-scoped historical resolution, distinct HRIS records, recorded-time assignment correction, covering employment, organization cycles, and candidate-worker continuity. It has no persistence or transport dependency and can be embedded by future services. This section describes active-PR work until merged into the protected default branch.
