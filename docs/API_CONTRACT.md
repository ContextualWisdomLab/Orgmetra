...
For People employment, position, and assignment HTTP writes, `evidence_references` is validated against the exact OpenAPI object contract: 1 through 100 unique objects, no additional properties, a non-empty `evidence_reference` of at most 500 characters, and a non-empty `evidence_version_code` of at most 200 characters. The runtime sorts the complete `(evidence_reference, evidence_version_code)` set and derives a PII-minimized `evidence_set_v1:<sha256>` binding code. That binding code is the downstream command evidence version used by the idempotency digest and immutable audit/outbox envelope. Reordering an unchanged evidence set is semantically identical; changing any reference or any version changes the binding and therefore cannot replay under the same idempotency key.
