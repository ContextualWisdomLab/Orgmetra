...
5. **Deliver**: asynchronous workers may mutate only guarded outbox delivery state. They may complete or retry only their exact live lease; after the immutable database-owned attempt budget is durably exhausted, that same live owner may terminally dead-letter the row only while atomically appending matching immutable escalation evidence. Workers cannot select a lower terminal threshold, fabricate nonterminal escalation evidence, reopen terminal rows, rewrite audit evidence, or infer a successful downstream receipt.
