...
```mermaid
flowchart LR
    a[Collect job evidence] --> b[Draft tasks, FJA and KSAO]
    b --> c[SME approves job profile]
    c --> d[Open requisition]
    d --> e[Collect candidate evidence]
    e --> f[Run structured assessment and interview]
    f --> g[Record evidence-backed selection decision]
    g --> h[Link candidate to worker]
    h --> i[Track assignments and performance criteria]
    i --> j[Run validation study]
    j --> k[Revise job profile or selection policy]
```
