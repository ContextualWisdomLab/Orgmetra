...
| **TAL-01 post-hire Talent ownership** | protected PRD/TRD/ARCHITECTURE stop Talent ownership at acquisition; Proposed ADR 0292/active PR #293 selects a dedicated post-hire owner and specifies immutable per-stage decision-opportunity/stage-transition denominators, exact opportunity-occurrence membership/counting-unit provenance, and #398 canonical correction resolution to one active terminal outcome per occurrence, but remains documentation-only | internal mobility, succession, talent pools and career-interest workflows otherwise risk being wedged into People or Acquisition truth; later fairness/adverse-impact analysis can also become irreproducible if only shortlist/survivor records remain, retries inflate the denominator, distinct repeated opportunities are silently collapsed, or corrected terminal history is double-counted | review/accept ADR 0292 without premature status promotion → normal protected integration → canonical context/schema/role/API/events/UI implementation → RED/GREEN opportunity-set/entry contracts preserving unavailable/excluded/suppressed/error states, exact semantic occurrence identity, counting unit, deterministic correction/supersession resolution, canonical terminal cardinality and relevant multiple-membership/cross-classified/repeated-opportunity/time structure → released cohort/provenance handoff to `workforce_validation` for explicit estimand/grouping → current-head security/PostgreSQL/E2E/a11y/i18n/recovery/performance evidence | **P1 buyer/scientific truth** |
