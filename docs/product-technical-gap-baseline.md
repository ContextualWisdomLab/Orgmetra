...
| **TAL-01 post-hire Talent ownership** | protected PRD/TRD/ARCHITECTURE stop Talent ownership at acquisition; Proposed ADR 0292/active PR #293 selects a dedicated post-hire owner and specifies immutable per-stage decision-opportunity/stage-transition denominators, exact opportunity-occurrence membership/counting-unit provenance, #398 canonical correction resolution to one active terminal outcome per occurrence, #399 explicit immutable manifest-lineage/version/as-of selection across replacement manifests, and #400 policy-regime/semantic compatibility so material selection-procedure changes cannot travel as ordinary corrections, but remains documentation-only | internal mobility, succession, talent pools and career-interest workflows otherwise risk being wedged into People or Acquisition truth; later fairness/adverse-impact analysis can also become irreproducible if only shortlist/survivor records remain, retries inflate the denominator, distinct repeated opportunities are silently collapsed, corrected terminal history is double-counted, replacement manifests silently rewrite or duplicate the denominator, or a changed eligibility/cut-score/ranking/evidence/routing procedure is presented as if only records were corrected | review/accept ADR 0292 without premature status promotion → normal protected integration → canonical context/schema/role/API/events/UI implementation → RED/GREEN opportunity-set/entry contracts preserving unavailable/excluded/suppressed/error states, exact semantic occurrence identity, counting unit, deterministic correction/supersession resolution, canonical terminal cardinality, stable manifest lineage/version/digest, explicit historical/as-recorded vs corrected-as-of view/cutoff, canonical policy-regime/semantic identity with deterministic compatibility evidence and relevant multiple-membership/cross-classified/repeated-opportunity/time structure → released cohort/provenance handoff to `workforce_validation` with exact selected manifest + governing policy regime for explicit estimand/grouping and scientific interpretation → current-head security/PostgreSQL/E2E/a11y/i18n/recovery/performance evidence | **P1 buyer/scientific truth** |
