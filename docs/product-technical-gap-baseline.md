...
5. **Employee Profile** — effective/system-time assignment history and correction semantics visible without exposing internal schema names.
...
- LLM output is draft/supporting evidence only. LLM work uses the contextual-orchestrator boundary, schema-bound outputs, model/provider discovery, explicit test-time compute policy and immutable provenance; it never receives direct authoritative employment-decision power.
