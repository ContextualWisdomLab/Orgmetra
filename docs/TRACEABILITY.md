...
| Governed audit/outbox envelope | Audit Provenance / Integration Hub contract | `AuditOutboxEvent` canonical CloudEvents envelope and SHA-256 digest; durable store still pending | high-impact confirmation, PII-minimized payload, timezone, namespace, governance-reference, and deterministic-digest regressions with exact 100% module statement/branch coverage | ADR-0006 | implemented_on_stacked_pr |
