...
| Tenant-safe atomic outbox claiming | Integration Hub dispatcher boundary | `outbox_delivery_record` claim index plus `claim_outbox_delivery(...)` | PostgreSQL expired-lease rejection, due-order claim, live-lease exclusion, tenant-context binding, opaque-worker validation, and bounded-lease contract | ADR-0006 | implemented_on_stacked_pr |
