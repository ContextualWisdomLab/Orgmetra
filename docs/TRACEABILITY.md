...
| Governed audit/outbox evidence | Audit Provenance / Integration Hub contract | `AuditOutboxEvent.canonical_json()`, `audit_outbox_record.event_envelope_text`, exact-byte SHA-256 digest, forced tenant RLS | envelope validation plus PostgreSQL atomic commit/rollback, digest recomputation, append-only, tenant mismatch, high-impact confirmation, and RLS regressions | ADR-0006 | implemented_on_stacked_pr |
