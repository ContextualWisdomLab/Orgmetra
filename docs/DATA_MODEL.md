...
| `candidate_application_record` | Immutable tenant-scoped application anchor binding one candidate to one opaque requisition correlation and submission/system-creation time. |
| `candidate_application_record_version` | Bitemporal Job and optional Job-consistent Position scope for one stable candidate application anchor; corrections append versions without changing application identity. |
| `candidate_application_stage_record` | Bitemporal non-terminal recruiting stage history for one stable candidate application; terminal employer outcomes and candidate withdrawal remain governed by separate evidence boundaries. |
...
Durable anchors such as `organization_unit`, `job_profile`, `employment_record`, `position_record`, and `candidate_application_record` do not repeat correctable descriptive or opening-scope attributes. Their mutable facts live in version relations. Single-valued bitemporal version families reject overlapping effective/system intervals, so one effective coordinate plus one knowledge coordinate cannot yield contradictory current descriptions. Corrections close the previous recorded version and insert a replacement; in-place business mutation is rejected. `candidate_application_record` is stricter still: its candidate/requisition identity is immutable and has no close-and-recreate correction path; opening-scope corrections belong to `candidate_application_record_version` so dependent stage history retains one stable application identifier.
...
Candidate application identity is distinct from candidate identity and from correctable opening scope. `candidate_application_record` keeps one immutable application anchor for one candidate/requisition context. `candidate_application_record_version` carries bitemporal Job and optional Position scope under that stable ID; adjacent recorded corrections are allowed while overlapping effective/system truth is rejected. `candidate_application_stage_record` carries effective and system-recorded intervals for non-terminal pipeline state and also references that stable ID. The stage vocabulary does not duplicate high-impact selection decisions or infer candidate withdrawal without separate governed actor/evidence provenance.
