...
Checked against the primary publishers on 2026-09-09. These references inform engineering choices; they do not imply certification or universal legal compliance.
...
- NIST SP 800-53 Rev. 5 AU-8 requires internal system clocks to generate audit-record timestamps and permits UTC or fixed-offset-compatible timestamp representation. The hierarchy-review leaf package validates canonical fixed-offset representation and rejects a future issuance timestamp, but its public API receives `recorded_at` from the invoking boundary. It therefore does not, by itself, prove AU-8 clock provenance. Authoritative mutation/audit persistence must generate or attest its own system-recorded audit/outbox time.
...
| Audit timestamp provenance stays with an internal system-clock owner rather than a caller-supplied leaf value | NIST SP 800-53 Rev. 5 Release 5.2.0, AU-8 Time Stamps |
