...
Five OpenAPI facts matter separately:
...
4. **Path Item shared semantics — Section 4.9.1.** A single Path Item Object contains operation definitions together with path-level `$ref`, `summary`, `description`, `servers`, and `parameters`. Path-level `servers` services all operations on that path; Path Item parameters apply to every operation and may be overridden but not removed by an operation. Therefore two separately released descriptions cannot be treated as one composed Path Item merely because their HTTP methods are disjoint unless the composition contract also proves how these shared fields are reconciled.
5. **Path Item operations — Section 4.9.1.** The 3.2 feature set includes the usual fixed HTTP operations plus `TRACE`, `QUERY`, and `additionalOperations` for additional methods.
...
Orgmetra also adds two narrower ownership rules. First, deterministic concrete-before-template overlap is admitted only when the concrete and templated routes bind the same exact owner release and declare the same exact method set. Second, one exact OpenAPI Path Item is bound to one exact `OwnerApiRelease` in the current profile even when its operations use disjoint methods. OpenAPI does not assign repository or bounded-context ownership; this second rule is a fail-closed product decision because the canary has no released cross-owner contract for reconciling Path Item-level `$ref`, servers, parameters, descriptions, or other shared semantics. Future cross-owner Path Item composition requires an explicit released merge/conformance contract rather than inference from disjoint methods.

Cross-owner concrete/template overlap remains fail-closed, and ambiguous overlap between two templated paths remains fail-closed. The same-method-set condition prevents composition from inventing a fallback to a templated operation when OpenAPI first selects a concrete Path Item that does not declare that method. Broadening this behavior requires released-owner conformance evidence, not framework-specific routing assumptions.

These OpenAPI rules remain separate from HTTP operation ownership. GET/HEAD collision equivalence is derived from RFC 9110, not OpenAPI. URI dot-segment rejection derives from RFC 3986. Orgmetra's additional evidence rule is that invalid, ambiguous, or cross-owner-unmergeable route material must fail before configuration identity is minted.
...
Draft #434 exact authority is `9ced14ae97f8c07285cdc3975f57a81c717313a2`, with 110 ordinary-forward commits / 17 changed files confined to `services/product-composition-api/**`, stacked on #340 exact `28f2bd28414e217f7e848ba86c0cfdbe97fd518f`.

Current executable evidence includes deterministic config hashing, canonical Orgmetra release attribution, pre-hash route-set validation, one release per owner service, owner-bound upstream identity, canonical lower-snake-case identities, URI dot-segment and repeated-expression rejection, one exact template identity per OpenAPI path hierarchy, deterministic same-owner concrete-before-template precedence, one exact owner release per exact Path Item, cross-owner/ambiguous-template rejection, separate GET/HEAD effective-authority checks, process-local receipt/source-generation integrity, deterministic receipt ordering, use-time graph revalidation, process-local generation lineage protection, and process-local construction identity for already-created owner releases and routes.

The newest construction-integrity sequences are:

- `0637353074d19327940b72821d497ffba131c284` -> `747c2a7a105e3abf23f8ddfbcb5b662f17e61924` -> `6744a55e95073b2c3f8572c62b84e555f580146a`: valid-to-valid low-level retargeting of an already-constructed `OwnerApiRelease` is rejected against a closure-private, lock-protected snapshot of all five release coordinates; a newly constructed successor remains legal;
- `5935135daf9eb2a89bcfd32e32105d9bd9563f96` -> `946d0a25faf5759233fb0aabeaf61f2fac653740` -> `9ced14ae97f8c07285cdc3975f57a81c717313a2`: the same distinction is enforced for `CompositionRoute` route ID, path, method tuple, exact owner release, logical upstream, and required/optional status.

The latest Path Item ownership sequence remains `ede0b6d602d2160af6c3ae37253f9728ac4b1e08` -> `8b7b059d60bf732e1acc33dc63db2ad8d5cbb52a` -> `9852fe6bf1b4cddf1b4710d2908cb42019a0ac86`. The prior standards-profile sequence remains `7a30ccd000c5e7b5a69e6f872277e7cf8fab2437` -> `a1b903cabc900b33ac4da36f3781bddc50cf57ee`. The deterministic-precedence sequence remains `fd7e7c69105fa2850dc30dd43014592f1a753eaa` -> `393cb561a1226529553baa6129378281cfba6f27` -> `a98dd5f7ffd175e2a7b3b5ca61255420f2959f8b`. The earlier path-key identity sequence remains `2b4f2211a2e332058bec31af11474d3937899855` -> `2c7bfcf75ec2d353d52ed86cbcd74dba5ddae3ea` -> `bf90bbfbc2eef88d57fd30286b41a86f483a3fa1`. Later test-only repairs aligned stale overlap assertions with the three-layer path model without weakening production admission.

The owner-release/route/generation construction rules are Orgmetra evidence-integrity requirements, not OpenAPI/OAuth/HTTP rules. Their weak-reference/lock implementation is a process-local mechanism, not cryptographic or durable provenance. GET/HEAD combines RFC 9110 semantics with an Orgmetra ownership decision. Dot-segment rejection combines RFC 3986 normalization with a fail-closed decision. Same-hierarchy identity, concrete-before-template precedence, repeated-expression validity, and Path Item shared-field semantics come from OpenAPI; the bounded route grammar/method whitelist, one-owner-per-Path-Item rule, and stricter overlap ownership rules are Orgmetra constraints.

Earlier focused local coverage figures at predecessor `cb32ba828...` are predecessor mechanism evidence only. Material source/test writes followed. Exact `9ced14ae...` currently has no PR-triggered hosted workflow evidence, so no current-head 100% coverage, Security, SAST, CodeQL, deployment, or release claim is made.
