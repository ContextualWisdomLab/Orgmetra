...
Draft #434 exact authority is `9852fe6bf1b4cddf1b4710d2908cb42019a0ac86`, with 88 ordinary-forward commits / 14 changed files confined to `services/product-composition-api/**`, stacked on #340 exact `28f2bd28414e217f7e848ba86c0cfdbe97fd518f`.

Current executable evidence includes deterministic config hashing, canonical Orgmetra release attribution, pre-hash route-set validation, one release per owner service, owner-bound upstream identity, URI dot-segment and repeated-expression rejection, one exact template identity per OpenAPI path hierarchy, deterministic same-owner concrete-before-template precedence, one exact owner release per exact Path Item, cross-owner/ambiguous-template rejection, separate GET/HEAD effective-authority checks, process-local receipt/source-generation integrity, deterministic receipt ordering, use-time graph revalidation, and process-local generation lineage protection.

The latest Path Item ownership sequence is:

- `ede0b6d602d2160af6c3ae37253f9728ac4b1e08` — regression-first contract: the same exact path template cannot be split between `people_api` GET and `job_analysis_api` PUT merely because their methods are disjoint; same-owner disjoint methods remain a positive control;
- `8b7b059d60bf732e1acc33dc63db2ad8d5cbb52a` — production admission now rejects a same-path `OwnerApiRelease` mismatch before operation-disjointness can allow the pair;
- `9852fe6bf1b4cddf1b4710d2908cb42019a0ac86` — README records Path Item-level shared semantics, the one-owner fail-closed decision, and the evidence threshold for any future cross-owner merge.

The prior standards-profile sequence remains `7a30ccd000c5e7b5a69e6f872277e7cf8fab2437` -> `a1b903cabc900b33ac4da36f3781bddc50cf57ee`. The deterministic-precedence sequence remains `fd7e7c69105fa2850dc30dd43014592f1a753eaa` -> `393cb561a1226529553baa6129378281cfba6f27` -> `a98dd5f7ffd175e2a7b3b5ca61255420f2959f8b`. The earlier path-key identity sequence remains `2b4f2211a2e332058bec31af11474d3937899855` -> `2c7bfcf75ec2d353d52ed86cbcd74dba5ddae3ea` -> `bf90bbfbc2eef88d57fd30286b41a86f483a3fa1`. Later test-only repairs aligned stale overlap assertions with the three-layer path model without weakening production admission.
...
Earlier focused local coverage figures at predecessor `cb32ba828...` are predecessor mechanism evidence only. Material source/test writes followed. Exact `9852fe6b...` currently has no PR-triggered hosted workflow evidence, so no current-head 100% coverage, Security, SAST, CodeQL, deployment, or release claim is made.
