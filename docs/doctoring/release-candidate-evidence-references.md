...
- The SBOM uses the required CycloneDX format/version identifiers and deterministic, unique `bom-ref` values. It records exact versions only when checked-in package metadata declares an exact version; non-exact Python/npm declarations keep the original declaration and use a stable digest-derived identity so ranges or markers are not conflated.
