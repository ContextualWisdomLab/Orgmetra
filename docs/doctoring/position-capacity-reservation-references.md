...
- Garcia-Molina and Salem define a saga as a sequence of transactions whose partial execution is amended by compensating transactions when the whole sequence cannot complete. ADR 0274 uses that as a design vocabulary for explicit compensation. It does not infer that a timeout, retry, absent message, point-in-time missing record, or later status read proves that another transaction cannot commit.
...
| Serialize competing capacity debits and eligibility-changing Position mutations at the stable Position root | PostgreSQL 18, §13.3 | `organization_core` owns the lock/transaction boundary for Position capacity and status/version eligibility; People does not query Organization application tables as correctness authority. |
| Model partial failure with explicit, auditable compensation/reconciliation | Garcia-Molina & Salem (1987) | `held`, `commit_fenced`, `confirmed`, terminal People attempt outcomes, and evidence-backed release/adjust transitions are domain state, not transport heuristics. |
| Keep a post-fence debit and eligibility promise until a terminal People attempt outcome exists | Saga compensation vocabulary plus Orgmetra's fail-closed seat/status invariants | TTL/HTTP timeout/missing event/current `not found` cannot release `commit_fenced`; release requires an `aborted` outcome that also fences every later commit for the same attempt, while a `committed` outcome requires confirm. |

## Evidence boundary

The sources above justify why the repository must explicitly control concurrency and compensation. They do not establish that ADR 0274's provisional choreography, People-owned attempt fence, or Position-eligibility fence is safe. Acceptance requires real two-service/PostgreSQL interleavings, delayed-create-versus-terminal-abort and fenced-create-versus-Position-status-change races, crash-window tests, idempotent replay tests, migration/rollback evidence, and exact-head quality/security/review gates listed in ADR 0274.
