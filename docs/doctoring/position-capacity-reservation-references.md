...
- Garcia-Molina and Salem define a saga as a sequence of transactions whose partial execution is amended by compensating transactions when the whole sequence cannot complete. ADR 0274 uses that as a design vocabulary for explicit compensation. It does not infer that a timeout, retry, absent message, or point-in-time missing record proves that a prior or delayed transaction cannot commit.
...
| Keep a post-fence debit until a terminal People attempt outcome exists | Saga compensation vocabulary plus Orgmetra's fail-closed seat invariant | TTL/HTTP timeout/missing event/current `not found` cannot release `commit_fenced`; release requires an `aborted` outcome that also fences every later commit for the same attempt, while a `committed` outcome requires confirm. |

## Evidence boundary

The sources above justify why the repository must explicitly control concurrency and compensation. They do not establish that ADR 0274's provisional choreography or its People-owned attempt fence is safe. Acceptance requires the real two-service/PostgreSQL interleavings, the delayed-create-versus-terminal-abort race, crash-window tests, idempotent replay tests, migration/rollback evidence, and exact-head quality/security/review gates listed in ADR 0274.
