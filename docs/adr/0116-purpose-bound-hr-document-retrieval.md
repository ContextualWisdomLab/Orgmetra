...
Execution order is security-significant: capability validation → request snapshot → fresh metadata resolution → exact-scope authorization → bounded read → artifact SHA-256 verification → immutable audit append → byte release.
