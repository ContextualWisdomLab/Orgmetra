...
`PurposeBoundAccessRequest` carries only authorization attributes: active tenant, authenticated actor tenant, resource tenant, opaque actor reference, purpose, requested operation, resource kind, requested field names, and granted operation scopes. `PurposeBoundAccessPolicy` is Orgmetra-owned and binds one tenant to one policy version, resource kind, purpose, operation, required Keyverse-derived scope, and immutable permitted field set.
...
- UUIDs, opaque actor references, codes, scopes, resource kinds, and field sets are syntactically explicit and reject reserved sentinels, wildcard-like values, ambiguous mutable collections, and malformed identifiers.

The authorization decision contains only governance metadata and authorized field names, never protected field values. Denials expose a stable reason code and a customer-facing next action without copying PII. An allow decision authorizes only the exact requested field subset; it does not widen the request to every field permitted by the policy.
...
- PII-minimized decisions can be bound into the governed audit/outbox envelope without duplicating protected field values.
...
- Callers must resolve trusted tenant/resource attributes before requesting protected values.
...
The executable authorization matrix proves same-tenant binding, resource/purpose/operation matching, operation-specific scope, exact field minimization, fail-closed malformed and mutable attributes, reserved UUID rejection, PII-minimized decisions, actionable denial metadata, and exception behavior. The owned `orgmetra_keyverse_adapter` production surface is required to maintain exact 100% statement and branch coverage where the pinned CI toolchain exposes those metrics.
