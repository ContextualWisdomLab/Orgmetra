...
- Status: proposed
- Scope: active PR #125 only; not protected-`develop` truth
...
2. `performance_goal_plan_version` is the bitemporal fact. It stores only goal-set and measurement-definition SHA-256 provenance, goal count, governed feedback cadence, exact plan/activation/authority evidence correlations, accountable approving actor, approval/activation chronology, business-effective interval and system-recorded interval.
...
Before accepting a version, the database re-resolves the same-tenant anchor, serializes mutation for that plan, requires complete system-visible `active|leave` Employment coverage and Job-version coverage for the business interval, and verifies an immutable same-tenant audit event plus transactional outbox correlation. The audit event must bind the plan reference, approving actor, activation-evidence digest, purpose `performance_goal_plan_persistence`, reason `activated_goal_plan_record`, result `activated_plan_persisted`, and activation instant.
