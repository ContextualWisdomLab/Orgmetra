...
- **Scope:** Orgmetra-owned audit envelope, immutable audit persistence, guarded outbox delivery state, tenant-safe atomic dispatcher claiming, expired-lease takeover, live-owner completion/retry, and retry-budget-governed terminal dead-letter escalation. Exponential retry policy, retention/export workflows, and external delivery receipts remain subsequent work.
...
8. Permits guarded `pending -> leased -> delivered` delivery transitions, `leased -> pending` retry recovery, only after lease expiry an atomic `leased -> leased` ownership takeover, and `leased -> dead_lettered` only through the governed terminal path after retry-budget exhaustion. Delivery identity and audit binding never change; delivered and dead-lettered records are terminal. Every new ownership grant must expire strictly after the transaction timestamp.
...
16. `dead_letter_outbox_delivery(...)` enforces the same tenant/live-owner capability boundary, validates an operational escalation id, lower `snake_case` failure code, namespaced opaque escalation reference, and configured maximum-attempt count from 1 through 100. It rejects terminal failure until the durable attempt count has reached that budget.
17. A successful dead-letter operation runs under the locked delivery row, inserts exactly one tenant-scoped `outbox_delivery_escalation_record` with the terminal attempt count/failure/reference, then transitions the delivery to `dead_lettered`, clears lease metadata, and preserves terminal failure evidence. The escalation relation is append-only under forced tenant RLS; normal claiming never selects `dead_lettered` rows.
...
- Permanent downstream failures can leave the normal dispatch queue only after an explicit attempt budget is exhausted, and the terminal failure leaves immutable tenant-scoped escalation evidence with a customer/operator reference.
