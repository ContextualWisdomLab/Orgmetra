...
13. Audit/outbox boundary functions pin `search_path = pg_catalog, public, pg_temp`; migration 0008 revokes `CREATE` on schema `public` from `PUBLIC`. This makes project-object resolution caller-independent while preserving the current application schema until a later schema-extraction migration explicitly changes it.
...
15. A partial `outbox_delivery_due_work_index` serves pending/leased queue scans without indexing terminal rows.
...
- Normal worker finalization remains capability-bound. A separate operator recovery mechanism prevents an exhausted final lease from becoming permanently stranded without allowing takeover of live or retryable work.
...
Review hardening added a focused PostgreSQL RED contract before migration 0008. That contract requires statement-level audit/outbox TRUNCATE denial, trusted search paths on representative database boundaries, deterministic/session-independent immutable envelope validation, the due-work partial index, and operator terminalization of only an expired exhausted lease with immutable operator-attributed escalation evidence. Additional review regressions make every captured dispatcher/dead-letter `psql` assertion fail on SQL errors, execute audit persistence sessions with explicit tenant context, assert the exact duplicate-delivery primary-key failure for transaction rollback, prove non-UTC application timestamps normalize to UTC, discover all executable migration/PostgreSQL-contract artifacts for provenance, and validate the database contract across all migrations rather than a hardcoded prefix.
...
See `docs/doctoring/REFERENCES.md` for CloudEvents v1.0.2, NIST SP 800-92, and PostgreSQL 16 locking, trigger, row-security, and date/time documentation in APA 7 style.
