...
Each live plan identity receives one process-local construction seal over its exact canonical bytes. Seal registration is one-shot: a repeated `__post_init__()` call cannot overwrite the original seal, including after low-level mutation to another syntactically valid value. Canonical export captures every serialized trust-bearing field exactly once, then validates exact built-in runtime types and renders canonical date/timestamp/JSON bytes from that same captured snapshot. It never validates one live observation and then rereads the object for serialization. That blocks representation-preserving executable aliases and checked-versus-used mutation between validation and rendering; a subtype present at capture fails closed before caller-controlled rendering can execute, while a later live mutation cannot alter the already-validated export snapshot. Export finally compares those canonical snapshot bytes with the original construction evidence instead of permitting reinitialization or representation-preserving aliases to renew trust. This runtime mechanism is defense-in-depth only and does not replace durable immutable audit/outbox evidence, persistence uniqueness, or cross-process authorization.
...
- A post-issuance runtime subtype cannot preserve evidence authority solely by keeping canonical JSON bytes unchanged, and mutation between runtime-type validation and rendering cannot substitute a different live value into the same export because both checks and serialization consume one captured snapshot.
