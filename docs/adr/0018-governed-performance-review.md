...
- opaque canonical non-sentinel UUIDv4-backed Person, Employment, Job, performance-cycle and performance-review references, rejecting UUIDv1 and other non-v4 suffixes;
...
`scope_verification_state` deliberately cannot be changed to `verified` inside this package. Before rating, the authoritative HRIS/performance boundary must resolve the Person↔Employment↔Job relation, performance-cycle/review-period alignment, and the governed evidence scope using its current temporal truth and purpose-bound authorization. UUIDv4 syntax is only an opacity constraint for packet-owned references; tenant UUID generation/version/privacy policy remains owned by the authoritative HRIS boundary.
