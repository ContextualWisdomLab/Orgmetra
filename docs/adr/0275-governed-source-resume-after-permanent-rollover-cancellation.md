...
After both adoption receipts are accepted, a single expected-transition-version CAS records an immutable `SourceAuthorityResumeReceipt` and moves the original rollover lineage from `source_resuming` to terminal state `source_resumed`. Because that receipt directly authorizes both owners to reopen HR business admission, it is durable write-authority evidence rather than an informational audit record. It therefore uses a versioned signed canonical envelope and binds at least `contract_version`, globally unique `receipt_id`, canonical issuer, the exact Organization and People audience set, tenant where applicable, transition identity and pre/post transition versions, terminal `source_resumed` state, source writer identity, old `(E,R)` and fresh `(E,R+1)` pairs, source-resume intent digest, cancellation-decision identity/digest and policy/evidence version, both exact adoption-receipt identities/digests, immutable signing-key identity/version, `domain_transition` purpose, and canonical payload digest. The verification-key manifest must authorize that key for the fresh pair and transition purpose; an audit-only or retired key cannot authorize gate reopening.

Only after signature and canonical-envelope verification may each owner open business admission for `(E,R+1)`. The gate-open compare-and-set is bound to the exact owner-local authority/gate version installed by that owner's adoption receipt and records the terminal resume receipt identity/digest as provenance. A byte-equivalent replay may observe an already-open matching gate idempotently. It must not refresh its expected gate version from current state: if a later transition has already closed or advanced that gate, replay of the old `SourceAuthorityResumeReceipt` fails the expected-version/state check and cannot reopen business admission. No request may ever obtain new `(E,R)` admission after the original closure.
...
13. A forged, unsigned, wrong-audience, stale-transition, wrong-pair, or audit-only-key `SourceAuthorityResumeReceipt` cannot open either owner gate. After a valid resume has opened `(E,R+1)` and a later transition closes that gate with a newer gate version, replay of the old valid resume receipt cannot reopen it; gate-open authority remains bound to the adoption-installed expected gate version and exact resume-receipt identity/digest.
