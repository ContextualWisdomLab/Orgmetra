...
- one tenant and one Orgmetra candidate-evidence reference;
...
Every envelope is explicitly `external_measurement_evidence`, `requires_human_review`, `score_values_not_stored`, and `not_authorized_for_employment_decision`. Any hiring or other high-impact HR action must re-resolve authoritative identity, purpose, evidence version, job/decision scope, and human authority at the owning Orgmetra decision boundary.
