...
The packet binds canonical tenant identity and UUID-backed opaque references to the Person, Employment, active Assignment/Job/Position scope snapshot, authoritative leave case, exact leave-policy version, work-continuity plan, benefits-continuity plan, return-to-work plan, exact personal-data handling policy, and exact retention policy. Every evidence artifact is independently bound by lowercase SHA-256. A bounded positive `evidence_version` is included in canonical evidence.
