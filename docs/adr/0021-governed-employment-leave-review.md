...
Portable review evidence is an especially poor place for medical/family details, free-form leave reasons, compensation/benefit values, credentials, or model-generated narrative. Those values must remain in their purpose-bound authoritative stores rather than being copied into a broad governance artifact.
...
Orgmetra will expose a value-free `EmploymentLeaveReviewPacket` before any authoritative employment/assignment status mutation associated with a leave workflow.

The packet binds canonical tenant identity and UUID-backed opaque references to the Person, Employment, active Assignment/Job/Position scope snapshot, authoritative leave case, exact leave-policy version, work-continuity plan, benefits-continuity plan, and return-to-work plan. Every evidence artifact is independently bound by lowercase SHA-256. A bounded positive `evidence_version` is included in canonical evidence.

The packet carries only non-sensitive workflow `reason_code` categories. It carries no substantive leave reason, medical/family values, Person PII, compensation or benefit values, credentials, or free-form model output. Opaque Person/Employment/leave-case references remain sensitive correlating metadata.
...
Immediately before approval, the host must re-resolve every packet reference in the exact tenant, prove requester/reviewer resolved identities are distinct, prove the Person-to-Employment and active Assignment/Job/Position scope represented by the snapshot, and verify the authoritative leave case, policy version, requested dates, continuity plans, and return-to-work provenance without copying sensitive case values into this packet.
...
- Buyers can correlate the exact leave case/policy and worker scope without copying medical/family or compensation/benefit values.
...
The package contract requires exact 100% owned statement and branch coverage, direct-construction and `dataclasses.replace(...)` fail-closed regressions, canonical precision-preserving timestamp/digest evidence, strict UUID-backed opaque references, non-sensitive reason categories, business-date ordering, bounded evidence version, explicit actor separation, privacy-value exclusion, immutable review/mutation/execution states, and a governed next action requiring authoritative resolution before approval.
