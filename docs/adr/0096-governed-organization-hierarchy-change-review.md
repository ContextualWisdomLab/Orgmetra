...
14. keeps module-level payload/canonicalization helpers outside the post-issuance verifier authority, so replacing `_payload` or a serializer after issuance cannot substitute stale pre-mutation evidence for the retained packet's direct slot state; and
...
- The top-level issuance semantic validator is selected at packet-runtime construction rather than through a later mutable module lookup, so ordinary module-name replacement cannot turn mandatory semantic validation into a no-op before sealing.
