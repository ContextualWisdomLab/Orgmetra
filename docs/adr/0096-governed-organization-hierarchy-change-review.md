...
9. reads every trust-bearing field exactly once into one local export snapshot, validates the exact built-in runtime types of that same snapshot, serializes only that snapshot, and then compares its deterministic canonical JSON SHA-256 with the issuance digest; this separates representation integrity from semantic mutation detection without a checked-versus-used gap; and
...
- Canonical export rejects a caller-defined scalar subtype observed at snapshot capture even when its JSON value and SHA-256 bytes are representation-preserving; if the live packet changes after snapshot capture, that export still validates and emits the same captured evidence, while a later export detects the changed live value against the issuance digest.
