...
11. independently captures one export snapshot, validates exact built-in runtime types on that same snapshot, serializes only that snapshot, and compares its deterministic canonical JSON SHA-256 with the issuance digest; this separates representation integrity from semantic mutation detection without a checked-versus-used gap;
12. binds each still-live `(tenant_record_id, organization_hierarchy_change_reference)` to one canonical evidence digest while allowing exact idempotent duplicate packets to share that binding, with the weak binding and packet-retention registries held in the same closure-private runtime state; and
...
- Creation sealing, export verification, and public digest reporting share one import-time-bound SHA-256 implementation inside the packet runtime, so ordinary replacement of the module digest name cannot make mutated evidence pass the creation-seal comparison.
...
- Process-local tamper/reference/digest detection is defense in depth only; durable uniqueness, authorization, concurrency control, hierarchy validation, authoritative system time and audit remain responsibilities of authoritative persistence/orchestration boundaries.