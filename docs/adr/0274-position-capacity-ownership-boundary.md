...
- make a post-fence receipt durable evidence of both consumed capacity and complete Position eligibility coverage for its effective interval, rather than requiring an unsafe People-side mutable recheck;
...
- `PositionCapacityReservation`: entity under the Position capacity authority. It binds an opaque reservation identifier, tenant, Position, allocation ratio, effective interval, opaque `assignment_attempt_id`, assignment intent/reference, canonical Position eligibility coverage evidence, idempotency key, semantic command digest, lifecycle state, and immutable audit/outbox correlation.
- `PositionEligibilityCoverage`: value object containing the canonical ordered set of visible Position-version interval slices whose union covers the reservation interval at the fence's knowledge cutoff. Each element carries the exact `position_record_version_id` and intersecting valid-time slice; the normalized set and cutoff are digest-bound so a reservation that spans multiple eligible Position versions remains reproducible without pretending one version covers the whole interval.
...
2. `arm_commit_fence`: under the same Position-root authority, revalidate capacity and complete `active`/`open` coverage for the exact effective interval, normalize the visible `PositionEligibilityCoverage`, and convert `held` to `commit_fenced`. This is durable, idempotent, coverage-digest-bound, non-expiring, and becomes an Organization promise not to commit an incompatible Position-status/version transition for any covered slice while the debit remains `commit_fenced` or `confirmed`.
3. `create_assignment`: after the Organization transaction has completed, People accepts only an authentic published/versioned Organization receipt proving the exact reservation is `commit_fenced` for the same tenant, Position, allocation, effective interval, semantic intent, complete Position eligibility coverage digest, and `assignment_attempt_id`. People does not perform a second mutable Position availability/status read as correctness authority. Inside one People transaction, it serializes the attempt identity and either writes the Assignment plus terminal `AssignmentAttemptOutcome=committed` atomically, returns the existing equivalent terminal outcome, or fails closed on contradictory evidence. A previously terminal `aborted` attempt can never later commit.
...
- a reservation spanning two or more contiguous staffable Position versions: the canonical eligibility-coverage set must cover the whole interval and an incompatible change to any covered slice must fail closed;
...
- Assignment correction/end and capacity/eligibility adjustment without leakage, overbooking, or an Assignment visible outside eligible Position coverage;
- a pre-fence legacy Assignment transaction deliberately held open while cutover starts: projection cannot begin until that transaction commits or rolls back, and no pre-fence transaction can commit after the migration snapshot;
- deterministic legacy projection replay: the same tenant-qualified Assignment snapshot produces identical migration attempt/reservation identities, terminal committed evidence, eligibility-coverage digests, and manifest digest;
- deliberately invalid legacy occupancy (for example overlapping allocation above `1.0000`, missing staffable Position coverage, or ambiguous tenant/identity mapping): cutover must fail closed with a deterministic discrepancy manifest, no authority switch, and no silent normalization of source facts;
...
- Eligibility evidence is a normalized coverage set, matching the protected rule that multiple visible staffable Position versions may jointly cover one Assignment interval; no arbitrary single-version shortcut is allowed.
...
- Organization also cannot independently close or otherwise invalidate a fenced effective slice while People can still commit from the authentic receipt; the same Position-root serialization governs capacity debits and eligibility-changing Position mutations.
...
- Cutover has an explicit transaction drain linearization point and a deterministic legacy Assignment-to-confirmed-reservation mapping. Projection cannot race a pre-fence legacy commit, and rerunning the same migration snapshot must reproduce the same manifest identities and digests.
- Invalid or unrepresentable legacy occupancy is surfaced as an immutable migration discrepancy and blocks authority switch; migration is not allowed to manufacture target validity by changing source truth.
