...
Replay protection is idempotency-preserving rather than timeout-based: byte-equivalent replay of the same `receipt_id` and canonical payload may return the same result, while reuse of that `receipt_id` with a different canonical payload, attempt ID, target, version, digest, issuer/audience, tenant, or authority epoch is contradictory evidence and fails closed. Superseded authority/rollback epochs are rejected even when the signature is otherwise valid. Receipt validity is not inferred from wall-clock age, network delivery state, or TTL because delayed legitimate recovery evidence must remain verifiable.
...
8. Switch Position/capacity write authority to `organization_core`, then switch `people_core` to the published capacity/attempt protocol, then remove the cutover fence. The authority switch increments the signed owner authority/rollback epoch pair used by the cross-context receipt contract; receipts from superseded writer epochs cannot authorize post-switch writes. No legacy transaction admitted before the fence remains able to commit after the projection or authority switch.

Rollback must be defined and rehearsed before un-fencing. It must use the same writer barrier, select one writer authority, increment the rollback/authority epoch before that writer resumes admission, and prove the chosen authority's projection is equivalent at its rollback cutoff; it must never re-enable both the legacy People Position/capacity writer and the Organization writer or accept receipts from the superseded epoch.
...
- byte-equivalent authenticated receipt replay is idempotent and does not depend on wall-clock expiry;
...
- migration projection equivalence and rollback rehearsal without a dual-writer interval, including rejection of receipts signed under superseded authority/rollback epochs;
...
- Cross-context write-authority receipts survive queues/retries with cryptographic source/integrity proof, explicit issuer/audience/tenant and authority epochs; equivalent replay stays idempotent while forged, stale-epoch, or contradictory replay fails closed.
