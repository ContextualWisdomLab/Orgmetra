...
2. MLX is never used inside containers. On Apple-silicon hosts, acceleration is provided by a native sidecar service (a separate process/binary executed on the host) that exposes the same governed request/response contract as the in-container kernel. The endpoint is an explicit deployment input, never implicit `localhost`:
   ...
   - Before accelerated work is accepted, the caller performs name resolution, connection, and a health/version handshake that proves the expected contract revision. Routing, connection, authentication, or version mismatch fails closed as `accelerator_unavailable`; there is **no silent CPU fallback** inside an already selected accelerated request. An operator may explicitly select the CPU deployment path and submit a new governed request.
...
- Deployment verification must prove the configured host route resolves from the actual container engine, the sidecar is not broadly exposed, the health/version handshake succeeds, and an unreachable/mismatched sidecar produces `accelerator_unavailable` without silently changing compute paths.
