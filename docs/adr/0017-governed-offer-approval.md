...
After all construction-time checks and UTC timestamp detachment succeed, the packet computes a process-local HMAC over the exact canonical JSON, stores that issuance seal in a non-init field, and registers the live packet identity for the lifetime of that object. Every later `canonical_json()` or `sha256_digest()` call recomputes the HMAC over the current fields and compares it in constant time with the issuance seal. A changed field, missing seal, or attempted second `__post_init__()` invocation fails closed instead of emitting changed canonical evidence. The registry therefore still rejects reissuance after low-level seal deletion. Standard shallow/deep copy return the same immutable issued object rather than minting a new evidence identity, and pickle serialization is rejected because the process-local issuance registry and random HMAC key intentionally cannot be transported as durable provenance across processes.

Canonical JSON and SHA-256 are audit-correlation evidence only. The process-local HMAC and identity registry are runtime mutation defense-in-depth only. Neither mechanism proves that referenced evidence is true, creates a durable signature or attestation, persists an offer, replaces immutable audit/outbox storage, or grants approval/delivery authority. The packet does not approve, communicate, send, execute, persist an offer, or prove authoritative reference/actor identity.
...
Within one Python process, post-issuance low-level mutation cannot silently produce a new canonical packet: evidence export fails closed, calling `__post_init__()` again cannot renew issuance even after seal deletion, and standard copy/deepcopy cannot mint a second renewable identity. Pickle is deliberately unsupported for this runtime evidence object rather than implying that process-local issuance state survives serialization. Durable integrity remains an owner-bound persistence concern; hosts must retain canonical bytes/digests and immutable audit/outbox evidence rather than treating runtime HMAC/identity state as a portable signature.
