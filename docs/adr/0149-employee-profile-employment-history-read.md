...
#55 is only the direct read-owner parent. Purpose-bound principal/policy/decision runtime integrity remains owned by PR #65 and its open security finding #175; #149 must not copy that mutable authorization implementation. Normal final integration therefore also requires #65 to reach protected `develop` through its own prerequisite chain, after which #149 adopts the protected authorization truth and reacquires exact-head evidence. The active #149 branch may continue source review while stacked on #55, but it is not authorized to treat the current protected authentication value object as the final commercial trust boundary.

The persistence executable is part of that authorization boundary. Before policy evaluation, the service resolves `read_employment_history` statically from the concrete repository type, rejects the Protocol placeholder or non-function descriptors, and retains that exact function object. After authorization it invokes the retained function directly with the repository instance rather than performing a new instance/class lookup. This matches the canonical People-read invariant owned by #55 and prevents caller-controlled `__getattribute__` behavior or authorization-time class mutation from substituting a different persistence capability after the access decision.
...
The checked persistence capability is frozen before authorization and used without a second dynamic lookup. A repository therefore cannot make the authorization decision apply to one executable lookup and cause a different method implementation to run afterward through `__getattribute__` or authorization-time class replacement. This is an in-process capability-integrity control; it does not make the persistence implementation trusted, so returned rows remain subject to all tenant, Person, temporal, type, shape, and scalar revalidation.
