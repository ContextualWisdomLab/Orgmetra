...
Persistence output is untrusted. Exact tuple and row types are only shape checks; a Python `frozen=True` dataclass is not an ownership boundary because code retaining the row can still use `object.__setattr__`. The service therefore reconstructs every accepted persistence row into a service-owned `EmploymentHistoryRecord` snapshot and uses only that detached snapshot for tenant/Person scope, system-time visibility, version uniqueness, business-time overlap checks, deterministic sorting, and authorized field emission. The reconstruction validates the copied primitive values before they can reach authorization-visible output, so a persistence-side alias cannot rewrite a value after validation and before use.
...
The persistence-alias boundary is treated as a local integrity concern rather than as evidence that the persistence adapter is malicious. Detachment prevents validation-to-use changes from an in-process adapter, fixture, cache, or other holder of the returned row reference without adding cross-service locks or weakening field minimization.
