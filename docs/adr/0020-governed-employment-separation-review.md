...
The packet binds canonical tenant identity and canonical non-sentinel UUIDv4-backed opaque references to the Person, Employment, an exact active Assignment/Job/Position scope snapshot, reviewed separation policy/process, value-free final-pay and benefits handoffs, access-deprovisioning, asset-return, knowledge-transfer, and communication plans. UUIDv1 and every other UUID version fail closed for namespaced trust references. Every evidence artifact is independently bound by lowercase SHA-256. A bounded positive `evidence_version` is included in canonical evidence so actor/purpose/reason evidence cannot silently drift across review-contract revisions.
...
- UUIDv1 timestamp/node correlation cannot enter namespaced trust references; UUIDv4 remains only an opaque syntax constraint, not proof of ownership or relationship.
