...
| Hosted exact-head proof | `.github/workflows/assignment-correction-quality.yml` | Exact checkout runs the focused HRIS-kernel, People command/HTTP/adapter, supersession, and replay-route contracts on the current candidate head. Absence, queueing, or predecessor results are not GREEN evidence. |
