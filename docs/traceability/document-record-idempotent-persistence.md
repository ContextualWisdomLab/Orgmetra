...
| Lost-response retry can recover authoritative identity and time | receipt persists in the same transaction as the document write | first committed result is followed by a separate retry that must return the same stored identity/receipt and original `recorded_at` instant; the returned epoch must equal both durable document and receipt epochs, not merely equal across the two calls | Implemented; hosted execution pending |
