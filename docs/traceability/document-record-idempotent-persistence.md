...
| Acceptance connections are closed | test sessions set dedicated `PGAPPNAME` values and are waited before inspection | both the concurrent root contract and post-commit recovery companion require `pg_stat_activity` to contain zero matching sessions after acceptance | Implemented; hosted execution pending |
