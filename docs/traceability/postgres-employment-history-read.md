...
7. **Capability-integrity RED:** `5a8c95f5942824d712a80ee4261ec320596605ad` adds a focused regression requiring the validated connection factory to resist post-construction replacement and remain the exact executable used by the read. Its parent still retained the dependency in a frozen/slotted dataclass slot, so the finding is causal rather than inherited from #55.
8. **Capability-integrity repair:** `a41ef5278ef45b15c1ab721244c40b35f828907a` replaces slot-backed dependency storage with immutable tuple payload, exposes only the retained capability view, and invokes the exact tuple-stored callable directly. This applies the same durable checked-versus-used invariant as canonical People reads without copying mutable #55 implementation source into this adapter.
9. **Retained-UUID RED:** `16dddcea47c60ceb0e2abc5154ffad5f03c484a2` adds two focused regressions. One forges an exact UUID's internal `.int` with executable equality and requires rejection before persistence. The other lets the connection factory mutate caller-owned tenant/Person UUID aliases after validation and requires the actual tenant-context and SELECT parameters to remain bound to the original identities.
10. **Retained-UUID repair:** `0d0993b3a9541cef4ddc7050df47ee55fc318a59` reads `.int` once, proves exact built-in integer authority and the operational range before comparison semantics, reconstructs detached UUID values before acquiring the connection, and uses only those detached values for SQL and post-read target verification.
11. **Transaction-integrity RED:** current-head review found that a callable/context-managed connection still could be autocommit. Test-first `b207e6db14452e36f2428c0fc472f8f5bb98e7a7` requires autocommit or otherwise unproven connection modes to fail before cursor access. `ab34fdcf0b02420219a1133571f87915e6934b8e` and `b1b51fbf241e768536dcbcb2c22a0e0cd443fee5` make accepted test doubles explicitly non-autocommit rather than relying on an unstated fake-connection assumption.
12. **Transaction-integrity repair:** `15c28cfaafd261bdea047961fb5461c641bb8be5` checks exact `autocommit is False` immediately after connection acquisition and before obtaining a cursor. PostgreSQL `SET TRANSACTION`, transaction-local tenant context, and the SELECT therefore execute only under a connection contract that actually provides an implicit transaction. Issue #318 hands the same verified defect to canonical governed-People owner #55 rather than copying this child implementation upstream.
13. **Hosted evidence rule:** #156 remains stacked on #155, while the canonical Foundation pull-request trigger targets `develop`. Therefore the source-level real-database contract and integrity repairs have not yet produced hosted exact-head acceptance evidence for this stack. After the owner stack reaches protected `develop`, #156 must retarget and reacquire Foundation plus applicable security/review gates; parent/predecessor results do not transfer.
