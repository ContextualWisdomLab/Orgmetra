...
The immutable connection-capability binding prevents a dependency that passed construction-time validation from being swapped through ordinary post-construction slot mutation before execution. Scalar detachment prevents forged retained UUID payload behavior and later caller-alias mutation from changing the database target. Neither control claims sandboxing or protection from arbitrary trusted-process code that fabricates unsupported objects outside the documented constructor boundaries.
