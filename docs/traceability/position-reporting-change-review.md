...
| Preserve business time vs system-recorded time | exact `effective_on` date is distinct from exact fixed-offset `recorded_at`, canonicalized to UTC; issuance rejects a system-recorded instant that has not occurred | fixed-offset, noncanonical temporal and future-`recorded_at` regressions |
...
| Prevent checked-vs-emitted runtime forgery and validation-hook bypass | caller-defined packet classes fail at subclass creation before they can replace `__post_init__`, `__getattribute__`, or another packet hook; exact built-in scalar primitives and the creation-time canonical seal remain separate boundaries | `test_packet_runtime_type_integrity.py`, hostile scalar runtime-subclass regressions, post-construction tamper regression |
