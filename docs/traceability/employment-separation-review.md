...
| Keep tenant and trust references opaque and non-correlating by UUID version | `tenant_record_id` and every namespaced packet reference require canonical non-sentinel UUIDv4 identity | `test_rejects_uuid1_tenant_identity`; `test_rejects_uuid1_trust_references_through_builder_and_replace` | implemented_on_active_pr |
