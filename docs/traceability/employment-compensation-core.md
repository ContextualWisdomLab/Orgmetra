...
| Keep compensation numeric truth finite enough for governed comparison | The fixed-scale `numeric(19,4)` type rejects PostgreSQL `Infinity` with a numeric overflow, while the amount constraint rejects negative values and `NaN`; review-driven regression proves neither special value can be persisted as a non-negative amount. |
