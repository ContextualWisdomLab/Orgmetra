...
| Keep compensation numeric truth finite enough for governed comparison | The amount constraint rejects negative values and PostgreSQL `NaN` or `Infinity`; review-driven regression proves these special values cannot be persisted as a non-negative amount. |
