...
| Keep trust references opaque and non-correlating by UUID version | every namespaced packet reference requires a canonical non-sentinel UUIDv4 suffix | `test_reference_guards_reject_uuid1_through_builder_and_replace` rejects UUIDv1 timestamp/node correlation across every trust-reference field and both construction paths | implemented_on_active_pr |
