...
| Bind the exact personal-data governance expected for the PII-bearing packet | UUID-backed `handling_policy_reference` (`personal_data_handling_policy`) and `retention_policy_reference` with independent SHA-256 digests; both participate in canonical JSON/SHA-256 and must be re-resolved before approval | reference/digest, policy-digest-change and immutable next-action regressions | implemented_on_active_pr |
