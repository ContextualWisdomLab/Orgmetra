...
| Prevent caller polymorphism and post-construction rewriting from changing checked-vs-emitted evidence | `test_rejects_caller_defined_runtime_subclasses`, `test_packet_runtime_type_integrity.py::test_rejects_packet_runtime_subclass_before_trust_field_validation`, `test_packet_runtime_type_integrity.py::test_rejects_subclass_that_bypasses_base_post_init`, `test_packet_runtime_type_integrity.py::test_rejects_representation_preserving_runtime_substitution_after_issuance`, `test_detects_post_construction_tampering_before_evidence_export` | active_pr |
...
Caller-defined packet classes are rejected at subclass creation, before a subclass can replace `__post_init__`, `__getattribute__`, or another validation/emission hook. Exact primitive checks reject scalar runtime substitution at issuance. Because `frozen=True` is not an unforgeable runtime boundary against low-level `object.__setattr__`, canonical export independently revalidates every trust-bearing scalar's exact built-in runtime type before serialization. The regression substitutes `str` and `int` subclasses that preserve the same JSON value; a digest-only guard would therefore miss them, while the runtime-type guard fails closed before evidence leaves the package.

Keeping an unreachable base packet-type branch after subclass creation was sealed would violate the package's owned 100% coverage contract, so packet-class finality lives at class creation. Scalar representation integrity is a separate export-time invariant and remains executable because a frozen instance can still be modified through low-level Python primitives.
