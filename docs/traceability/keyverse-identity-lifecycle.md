...
Status: **active PR only**. Protected `develop` does not yet contain this review packet.
...
| Reject validation-forging runtime types | exact strings/integers/datetimes, final evidence type | hostile/wrong runtime, UUID, digest, version, time and subclass regressions | Orgmetra-owned | implemented_on_active_pr |
