...
| Canonical OpenAPI and buyer performance journey | #314 | `schemas/openapi.yaml` must publish the separation request/result/error contract and the exact route must obtain buyer-path p95/E2E evidence without exposing owner-role or direct-DML capabilities | planned |
...
The remaining buyer-visible gap is to publish the already-implemented HTTP command in the canonical OpenAPI contract, add exact schema/route regression coverage, and then measure the route under the buyer-path performance/E2E lane. No performance claim is made from unit tests or queued CI alone.
