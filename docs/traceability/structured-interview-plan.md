...
| Portable governance metadata is value-minimized | `tenant_record_id` and all trust-bearing references require canonical non-sentinel UUIDv4 identity; namespaced references also require their expected prefix; reason is closed to `approved_requisition_interview` | tenant UUIDv1 regression, scalar/collection direct-constructor privacy regressions, UUIDv1 reference regressions, and `dataclasses.replace(...)` bypass regression |
