...
The rehearsal creates an isolated source database on one PostgreSQL service, applies every protected-main migration in `database/migrations` in numeric order, writes a tenant-bound bitemporal Person/Name fact plus immutable audit/outbox evidence, produces a real PostgreSQL custom-format dump, and restores it into a separate exact restored database on a second independent PostgreSQL service. That separation prevents a same-cluster rehearsal from silently inheriting cluster-global roles or other state from the source. The restored state, not the source state, is then validated.

The rehearsal derives its migration set from the checked-in `database/migrations` directory rather than a frozen list, so a newly added protected-main migration is exercised by recovery evidence without a second edit. Discovery happens before any cluster connection or destructive DDL: it fails closed when the directory is missing, unreadable, or empty, when a `.sql` basename is not canonical `^[0-9]{4}_[a-z0-9_]+\.sql$`, or when two migrations share a numeric prefix. The executable recovery test binds that behavior to the directory contents and to discovery failure: it fails closed if the rehearsal applies a migration twice, skips one, applies them out of order, or fails to discover the migration set at all.
...
| Complete protected-main migration sequence | The rehearsal enumerates `database/migrations` in numeric order and fails closed before any connection or DDL when discovery is missing/empty/non-canonical or has duplicate prefixes; the executable contract diffs the applied paths against the directory and proves discovery failure cannot reach the `psql -f` application loop | Every protected-main migration is applied exactly once, in order, and a skipped, reordered, undiscovered, or ambiguous migration set fails closed | implemented_on_active_pr |
