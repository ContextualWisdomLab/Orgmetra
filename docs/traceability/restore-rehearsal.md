...
| Complete protected-main migration sequence | The rehearsal iterates `database/migrations` in numeric order; the executable contract diffs the applied paths against the directory | Every protected-main migration is applied exactly once, in order, and a skipped or reordered migration fails closed | implemented_on_active_pr |
