...
- **Active PR truth:** governed non-authoritative Job grade/band design review packet and its exact-head quality gate.
- **Planned after integration:** authoritative bitemporal Job-grade persistence with tenant isolation and immutable audit/outbox; purpose-bound API/UI access; accessible Storybooked/Figma product interaction when material.
...
Before any reviewed proposal is persisted as HRIS truth, re-read the exact tenant Job, persisted Job Analysis snapshot, evaluation-method definition, and enterprise grade/band architecture; verify all digests and reviewer authority; then write one bitemporal Job-grade fact and immutable audit/outbox evidence in a single Orgmetra-owned transaction. Do not infer compensation or an employment outcome from the review packet.
