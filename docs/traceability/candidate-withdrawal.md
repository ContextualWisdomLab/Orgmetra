...
| Application chronology is preserved | withdrawal cannot predate application `submitted_at`; audit time must equal `withdrawn_at` and not exceed database-owned `recorded_at`, which the insert trigger assigns from transaction time | migration trigger exercised by happy-path, caller-future-time, and anti-forgery contracts |
