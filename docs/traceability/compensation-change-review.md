...
| Prevent reference fields from becoming a PII/value/correlation smuggling channel | Every namespaced trust-bearing reference is the expected namespace plus canonical non-sentinel UUIDv4; UUIDv1 and other UUID versions fail closed | `test_invalid_core_inputs_fail_closed`, `test_every_reference_and_digest_is_validated`, `test_uuid1_trust_reference_is_rejected_by_builder_and_replace` |
