...
| Server errors are aggregatable without backend exception disclosure | HTTP 5xx uses its decimal status string; pre-status unhandled errors use `unhandled_exception`; a missing response start uses `missing_response_status` | `test_records_server_error_as_low_cardinality_status_error_type`, `test_unhandled_exception_is_measured_then_reraised`, `test_missing_response_start_is_bounded_operational_error` |
