...
- **This active-PR truth:** #128 owns the durable application/persistence boundary. It is a dependency-first Draft and is not shipped product truth.
- **Out of scope:** payroll calculation, compensation mutation, statutory leave classification, scheduling, disability/medical case data, Assignment mutation, candidate decisions, or foreign-service table access.
...
| Concurrency safety | transaction advisory lock on tenant + Employment before current-state validation | design requirement in ADR 0131; focused gate must remain GREEN after future concurrency expansion |
...
| Correction not rewrite | only database-time closure of `recorded_to` is allowed | UPDATE/DELETE/TRUNCATE adversarial regressions |
