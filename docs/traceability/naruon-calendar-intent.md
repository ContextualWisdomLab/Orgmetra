...
The transport-neutral calendar-intent adapter is protected-`develop` product truth. The current runtime-integrity hardening remains active-PR truth until its owning PR integrates.
...
| Require actor, purpose, reason, evidence version, tenant context, exact HR resource and explicit human confirmation before an external calendar intent | ADR 0010 | exact `CalendarIntentContext` plus `_validate_context`; trust-bearing text validators require built-in `str` values so caller-controlled subclasses cannot forge validation/allowlist behavior while retaining different audit evidence | malformed-context, human-confirmation, action-kind, UUID/reference and `test_runtime_integrity.py` adversarial regressions |
| Do not leak HR identifiers or free-form PII into calendar summary text | ADR 0010 | fixed `_ACTION_SUMMARIES`; audit context is kept separate from request body; action-kind lookup occurs only after exact built-in code validation | `test_builds_confirmed_intent_without_pii_or_provider_execution`; forged-action regression |
| Keep audit correlation equal to the resource that actually crossed validation | ADR 0010 | exact built-in resource-reference validation rejects subclasses before namespace parsing or audit-context construction | forged-resource-reference regression in `test_runtime_integrity.py` |
...
| Fail closed on Naruon contract/provenance drift | ADR 0010 | `validate_calendar_intent_response` requires exact response/provenance keys, exact built-in fixed literals for CalDAV/customer-owned mode, create semantics, target consistency, intent-only status and bounded metadata | response-drift parameter matrix plus auto-selected-target and forged-response-literal regressions |
| Keep local adapter evidence exact and independently reproducible | Orgmetra quality rules | `.github/workflows/naruon-adapter-quality.yml` checks exact candidate SHA, reviewed hashed test dependencies, compilation, 100% statement/branch coverage and clean checkout | hosted `Naruon Adapter Quality` run on each exact PR head plus local RED/GREEN development evidence |
