...
- `dead_letter_outbox_delivery(...)` is the only supported terminal failure function. It row-locks the delivery, requires the active tenant and exact recorded owner, reads the stored `maximum_attempt_count`, and rejects dead-lettering until durable `delivery_attempt_count` reaches that budget. Before exhaustion an expired owner must reclaim normally; after exhaustion the exact recorded stable worker reference may terminalize its expired final lease so a restarted worker can finish escalation without creating attempt N+1. A foreign worker cannot claim or terminalize that exhausted row.
...
- Stable dispatcher worker references are therefore operational identities, not random per-request values. A restarted final-attempt worker must reuse its recorded opaque worker reference to close an exhausted expired lease; changing that identity intentionally requires a separate, audited future operator-takeover contract rather than direct row mutation.
- Exponential/backoff policy selection, policy-specific producer configuration, audited operator takeover for a permanently lost final worker identity, and external delivery receipts remain release blockers before reliable asynchronous delivery is called production-ready; terminal dead-letter/escalation evidence is implemented but does not by itself prove downstream receipt.
...
- lost final-attempt dispatcher identity requiring audited recovery
- integration outage
