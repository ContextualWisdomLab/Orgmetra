...
- Migration 0008 builds `outbox_delivery_due_work_index` with `CREATE INDEX CONCURRENTLY` so an established queue can continue accepting inserts, updates, and deletes during the build. Because PostgreSQL forbids concurrent index creation inside an explicit transaction block, deployment runners must execute that migration in autocommit/non-transactional mode and treat any invalid concurrent-index residue as a failed migration requiring operator inspection before retry.
...
- Operator recovery uses two service-owned NOLOGIN roles. `orgmetra_outbox_recovery_owner` owns the single `SECURITY DEFINER` recovery function, is explicitly `NOBYPASSRLS`, loses schema `CREATE` immediately after function ownership is assigned, and receives only the transport-table privileges required by that function. `orgmetra_outbox_operator` receives only schema usage and EXECUTE on the recovery function, with no direct outbox UPDATE or escalation INSERT privilege. Production login identities obtain this capability only through explicit purpose-bound membership provisioning; migration execution therefore requires authority to create or harden these service-owned roles.
