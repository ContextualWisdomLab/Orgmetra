...
- Expired owners must recover through `claim_outbox_delivery(...)` before completion or retry. This prevents a worker that lost ownership from racing the replacement owner after lease takeover.
- Dead-letter/escalation policy, exponential retry policy, and external delivery receipts remain release blockers before reliable asynchronous delivery is called production-ready.
