...
- `outbox_delivery_record` is transport state. A dispatcher may move `pending -> leased -> delivered`, return `leased -> pending` with a bounded failure code and cleared lease metadata for retry, atomically replace an expired `leased` ownership grant with a new `leased` grant, or move an exhausted live lease to terminal `dead_lettered`. Delivery identity and audit binding never change.
...
- `dead_letter_outbox_delivery(...)` is the only terminal failure path. It row-locks the delivery, requires the active tenant and exact owner of a still-live lease, validates a 1–100 configured maximum-attempt budget, and rejects dead-lettering until the durable `delivery_attempt_count` has reached that budget.
...
- Exponential/backoff policy selection and external delivery receipts remain release blockers before reliable asynchronous delivery is called production-ready; terminal dead-letter/escalation evidence is implemented but does not by itself prove downstream receipt.
