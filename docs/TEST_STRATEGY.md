...
| Audit/outbox hardening: exact execution capability, guarded privileged recovery, reserved-role collision rejection, search-path safety, append-only enforcement including TRUNCATE, and recovery-boundary invariants | `bash tests/test_audit_outbox_hardening_postgres.sh` against PostgreSQL 16 in Foundation CI |
...
| Candidate-to-worker conversion governance: exact candidate/person/employment/selection lineage, sealed evidence, human confirmation, audit/outbox binding, tenant isolation, and fail-closed conversion invariants | `bash tests/test_candidate_worker_conversion_postgres.sh` against PostgreSQL 16 in Foundation CI |
| People mutation idempotency: same tenant+route+key cannot persist a second employment/assignment/audit/outbox fact, confirmed-hire route keys are accepted by the durable route constraint, a different key is a new command, rollback leaves no replay marker, and tenant RLS isolates keys | `bash tests/test_people_mutation_idempotency_postgres.sh` against PostgreSQL 16 in Foundation CI; `PYTHONPATH="packages/hris-kernel/src:services/keyverse-adapter/src:services/people-api/src:$PYTHONPATH" pytest services/people-api/tests --cov=orgmetra_people_api --cov-branch --cov-report=term-missing --cov-fail-under=100` for exact 100% owned People API statement/branch coverage |
