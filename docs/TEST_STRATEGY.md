...
| Governed terminal outbox failure: premature dead-letter rejection, exact live-owner enforcement, explicit attempt-budget exhaustion, terminal non-reclaimability, immutable escalation evidence, and normalized tenant-scoped escalation linkage | `bash tests/test_outbox_dead_letter_postgres.sh` against PostgreSQL 16 in Foundation CI |
...
- dead-lettering must require the active tenant, exact owner of a still-live lease, a bounded 1–100 maximum-attempt policy, durable budget exhaustion, lower `snake_case` failure classification, and opaque escalation reference; normal claiming cannot reopen the terminal row and escalation evidence rejects update/delete;
