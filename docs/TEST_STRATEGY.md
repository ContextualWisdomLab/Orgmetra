...
| Reason-free Employment absence persistence: same-tenant Employment/Person binding, database-owned time, confirmed coverage, post-coverage cancellation correction, bitemporal non-overlap, append-only/TRUNCATE protection, forced RLS, forbidden sensitive columns, and trusted function search paths | `bash tests/test_employment_absence_persistence_postgres.sh` against PostgreSQL 16 in the Employment Absence Persistence workflow |
