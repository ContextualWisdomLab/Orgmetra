...
| People mutation idempotency: same tenant+route+key cannot persist a second employment/assignment/audit/outbox fact, a different key is a new command, rollback leaves no replay marker, and tenant RLS isolates keys | `bash tests/test_people_mutation_idempotency_postgres.sh` against PostgreSQL 16 in Foundation CI plus People API mutation contracts at 100% owned statement/branch coverage |
