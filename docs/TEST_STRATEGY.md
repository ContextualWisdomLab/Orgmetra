...
The command runs Python repository-integrity validation, the dependency-free Node foundation validator, Node regression tests, and mutation-style OpenAPI operation-contract tests. It must fail on a missing required artifact, manifest mismatch, invalid database name, missing tenant/evidence/audit/temporal/candidate-conversion DDL contract, incomplete high-risk OpenAPI operation context, empty OpenID Connect scope requirement, internal trace identifier in a client error schema, explicit unfinished-work marker, unbalanced Markdown fence, or incomplete Apache-2.0 license. Ordinary explanatory prose may contain words such as `placeholder`; only explicit TODO/TBD/FIXME marker forms are treated as unfinished work.

## Foundation test matrix

| Evidence | Execution command |
|---|---|
| Required artifacts, manifest SHA-256/byte/line integrity, package metadata, Markdown, license, database naming, tenant/evidence/audit/dispatcher/candidate-conversion DDL fragments | `npm run validate` |
...
| Every executable PostgreSQL migration and contract remains present in both Python and Node provenance inventories | `node --test tests/dispatcher-inventory.test.mjs` (also included in `npm run validate`) |
...
| Governed candidate-to-worker conversion: legacy direct-link denial, same-candidate human-confirmed hire decision, sealed versioned evidence, employment/person consistency, exact immutable audit/outbox binding, one current system-knowledge conversion, non-hire and candidate-mismatch denial, atomic rollback on rejected conversion, bitemporal correction with correction-specific audit semantics, immutable-history enforcement, and forced tenant RLS | `bash tests/test_candidate_worker_conversion_postgres.sh` against PostgreSQL 16 in Foundation CI |
...
- a legacy `candidate_worker_link` insert cannot create new worker lineage;
- candidate-to-worker conversion requires the same candidate's human-confirmed `hire` decision, a tenant-local employment owned by the resulting person, a sealed versioned evidence set with at least one member, and an exact immutable high-impact audit/outbox event;
- a second candidate conversion cannot coexist at the same system-knowledge coordinate, even if its business-effective interval would not overlap the first;
- a conversion correction must close the prior recorded interval, append a replacement version, and bind a correction-specific immutable audit event rather than replay the original worker-created event;
- a rejected conversion transaction cannot leave an orphan audit/outbox event when the business mutation, audit, and outbox are issued atomically;
