...
| Reviewed Organization hierarchy application: exact review JSON types/nulls/next action, tenant/unit-qualified predecessor and deferred successor bindings, evidence-column matching, future-effective cycle rejection, bitemporal preservation, immutable audit/outbox evidence, forced RLS, and untrusted executor denial | `bash tests/test_organization_hierarchy_change_application_postgres.sh` plus `bash tests/test_organization_hierarchy_change_application_concurrency_postgres.sh` against PostgreSQL 16 in the dedicated quality workflow |
...
- a reviewed Organization hierarchy application rejects malformed JSON types/nulls/arbitrary next actions, cross-unit predecessor bindings, missing or unrelated successors, stale reviewed columns, and cycles that emerge at a later effective-time boundary;
