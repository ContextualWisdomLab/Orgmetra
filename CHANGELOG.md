...
- Database-level rejection of cross-tenant references, post-decision evidence insertion, caller-supplied open-set evidence digests, empty decision evidence, sealed evidence-set reuse, digest-tampered audit envelopes, non-allowlisted audit payload fields, high-impact audit events without confirmation, illegal outbox state transitions, already-expired new dispatcher leases, live-lease theft, cross-tenant outbox claims, unsafe lease-owner identifiers, foreign-owner completion/retry/dead-letter, stale-owner finalization after lease expiry, premature dead-lettering before retry-budget exhaustion, mutation of terminal escalation evidence, and RFC 9562 Nil/Max UUID sentinels across foundation and audit/outbox identities.
- Expired dispatcher ownership is recoverable only after the recorded lease deadline; takeover preserves immutable event/audit identity, increments the attempt count, issues a new future lease, and records `lease_expired` failure evidence.
- Dead-lettering is permitted only to the exact owner of a still-live lease after an explicit 1–100 attempt budget is exhausted; the terminal failure is removed from normal dispatch and leaves one immutable tenant-scoped escalation record with failure classification, terminal attempt count, and opaque external reference.
...
- The protected default branch contains only the minimal bootstrap commit. The canonical foundation is PR #22; the governed audit/outbox persistence, dispatcher claim/crash-recovery, owner-bound completion/retry, and retry-budget dead-letter escalation slice is stacked on that exact head and is not protected-main truth until dependency order and fresh merge gates are satisfied.
