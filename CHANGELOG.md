...
- Stacked candidate-to-worker conversion governance via migration 0008: closes new writes to legacy `candidate_worker_link` and adds `candidate_worker_conversion_record` with tenant-scoped candidate/person/employment binding, sealed human-confirmed hire-decision provenance, versioned decision evidence, effective/business and recorded/system time, bitemporal overlap rejection, immutable-history correction rules, and forced tenant RLS. Historical legacy links remain explicitly legacy rather than receiving invented provenance.
- Stacked governed audit/outbox slice via `AuditOutboxEvent`, `audit_event_record`, `outbox_delivery_record`, and `outbox_delivery_escalation_record`: CloudEvents 1.0-compatible PII-minimized metadata, exact canonical JSON bytes, database-verified SHA-256 digests, mandatory human confirmation for high-impact events, immutable audit evidence, tenant RLS, atomic audit/outbox insertion, guarded pending/leased/delivered/dead-lettered delivery state, tenant-safe `claim_outbox_delivery(...)` with deterministic due-work ordering, `FOR UPDATE ... SKIP LOCKED`, opaque worker identity, bounded future leases, immutable envelope return, and atomic takeover of genuinely expired leases only while retry attempts remain; owner-bound `complete_outbox_delivery(...)` and `retry_outbox_delivery(...)`; and database-budget-governed `dead_letter_outbox_delivery(...)` that records append-only escalation evidence and excludes terminal failures from normal dispatch. `maximum_attempt_count` is persisted on the delivery row, defaults to 5, is constrained to 1 through 100, and cannot be lowered by a dispatcher during finalization. Migration 0007 prevents retry or expired-lease takeover from creating attempt N+1 and lets only the exact recorded stable worker identity terminalize its exhausted final lease after expiry. Exponential/backoff policy selection, policy-specific producer configuration, audited operator takeover for a permanently lost final-worker identity, and external delivery receipts remain subsequent work.
...
- Database guards for reversed or zero-length temporal intervals and append-only legacy candidate-worker, selection-decision, decision-evidence, validation-study linkage, audit-event, and outbox-escalation records; governed candidate conversion uses bitemporal correction semantics instead of mutable linkage.
- Database-level rejection of cross-tenant references, post-decision evidence insertion, caller-supplied open-set evidence digests, empty decision evidence, sealed evidence-set reuse, ungoverned candidate-to-worker conversion, non-hire candidate conversion, candidate/decision mismatch, conversion before the hiring decision, candidate conversion without sealed versioned evidence or explicit human confirmation, digest-tampered audit envelopes, non-allowlisted audit payload fields, high-impact audit events without confirmation, illegal outbox state transitions, already-expired new dispatcher leases, live-lease theft, cross-tenant outbox claims, unsafe lease-owner identifiers, foreign-owner completion/retry/dead-letter, stale-owner completion after lease expiry, dispatcher-supplied terminal retry thresholds, retries and claims beyond `maximum_attempt_count`, direct premature dead-letter DML, fabricated escalation evidence for nonterminal delivery state, mutation of terminal escalation evidence, and RFC 9562 Nil/Max UUID sentinels across foundation and audit/outbox identities.
...
- Dead-lettering is permitted only to the exact recorded worker identity after the immutable database-owned retry budget is exhausted; the transition guard also requires matching escalation evidence, the terminal failure is removed from normal dispatch, and one tenant-scoped append-only escalation record preserves failure classification, terminal attempt count, and opaque external reference. The recorded identity may finish terminalization after its exhausted final lease expires, preventing attempt N+1 without granting another worker takeover authority.
- Bitemporal reconstruction plus assignment, position-seat, employment-exclusivity, and candidate-conversion decisions are tenant-scoped so foreign-tenant identifiers cannot leak historical facts, provide coverage, consume capacity, create false conflicts, or authorize worker creation.
...
- Protected `bootstrap` currently contains the integrated foundation through PR #19 at `e241793d2b0784c6f2ea8f226a9fa0d4b925f573`. PR #23 provides the governed audit/outbox slice at `c78aed423ae5c28b54caf6da389805ba5addce16` and is ready for qualifying independent review but is not protected-main truth. Candidate conversion governance PR #24 is stacked on that exact #23 head and remains Draft until dependency order is satisfied and fresh exact-head gates are obtained.
